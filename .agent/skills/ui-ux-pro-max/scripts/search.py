...
def _load_local_core():
    """Load sibling core.py without cross-module imports."""
    core_path = Path(__file__).resolve().parent / "core.py"
    spec = importlib.util.spec_from_file_location("uiux_local_core", core_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Cannot load local core.py for ui-ux-pro-max")
...
CSV_CONFIG = _CORE.CSV_CONFIG
AVAILABLE_STACKS = _CORE.AVAILABLE_STACKS
MAX_RESULTS = _CORE.MAX_RESULTS
search = _CORE.search
search_stack = _CORE.search_stack
