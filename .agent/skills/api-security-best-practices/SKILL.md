...
description: "Comprehensive API security covering OWASP Top 10, authentication patterns (JWT, OAuth2, API keys), authorization (RBAC, ABAC), input validation, parameterized queries, rate limiting, DDoS protection, secure headers, error handling, and vulnerability remediation. Includes JWT best practices, refresh token patterns, password hashing, CORS configuration, request validation with Zod, SQL injection prevention, XSS sanitization, and security checklist for REST/GraphQL/WebSocket APIs. Use when securing API endpoints, designing authentication systems, implementing authorization checks, protecting against injection attacks, preventing DDoS, conducting security audits, or remediating API vulnerabilities."
type: feature
---
