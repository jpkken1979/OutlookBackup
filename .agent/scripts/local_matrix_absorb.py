...
- Tags canonicos segun .claude/rules/repo-crawler.md.
...
IMPORTANT_FILENAMES = {
    "CLAUDE.md", "README.md", "AGENTS.md", "RULES.md", "GEMINI.md",
    "ESTADO_PROYECTO.md", "WORKFLOW_RULES.md", "SPEC.md", "PLAN.md",
...
MODEL_GLOBS = [
    "**/prisma/schema.prisma", "**/models.py", "**/models/*.py",
    "**/schema*.py", "**/types.ts", "**/types/*.ts",
    "**/*_model.py", "**/*.model.ts", "**/entities/*",
    "**/migrations/*.sql", "**/db/schema*", "**/*.prisma",
]

ROUTE_GLOBS = [
    "**/routes.py", "**/routes/*.py", "**/api/*.py", "**/api/*.ts",
    "**/app/api/**/route.ts", "**/pages/api/**/*.ts",
    "**/endpoints.py", "**/controllers/*",
]

SKIP_DIRS = {"node_modules", ".git", "target", "dist", "build", ".next",
    ...
    for p in app_dir.rglob("*"):
        ...
        if name == "package.json" and not result["tech_stack"]:
            try:
                ...
                for fw in ("next", "react", "vue", "svelte", "vite", "tauri",
                           "electron", "astro", "angular", "express", "fastify"):
...
def main() -> None:
    ...
    parser.add_argument("--missing-only", action="store_true",
                        help="Solo apps sin nodos 'local-scan' previos")
    parser.add_argument("--skip-large", type=int, default=5000,
                        help="Saltar apps con mas de N archivos (default 5000)")
    args = parser.parse_args()

    brain = Brain(BRAIN_DIR, app_id="nexus-mother")

    to_process = list(APPS.items()) if not args.only else [
        (args.only, APPS[args.only])
    ] if args.only in APPS else []
    ...
    for app_name, info in to_process:
        ...
        logger.info("  Files=%d Tech=%s Models=%d Routes=%d Docs=%d",
                    scan["file_count"], scan["tech_stack"],
                    len(scan["models"]), len(scan["routes"]), len(scan["docs"]))
    ...
    logger.info("=== DONE: procesados=%d skipped=%d nodos_nuevos=%d ===",
                processed, skipped, total_nodes)
