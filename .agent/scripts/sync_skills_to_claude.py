#!/usr/bin/env python3
"""Sync Antigravity skills to Claude Code native skills.

Copies `.agent/skills/*/SKILL.md` and `.agent/skills-custom/*/SKILL.md`
to `.claude/skills/{skill-name}/SKILL.md` so that Claude Code recognizes
them natively.
...
def sync_skills(
    ...
    for skill_dir in skill_dirs:
        ...
        if dry_run:
            logger.info(f"  [DRY] Would create: {skill_path}")
            stats["created"] += 1
            continue
