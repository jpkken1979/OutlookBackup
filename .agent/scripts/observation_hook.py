...
INDEX_CHROMA = os.environ.get("ANTIGRAVITY_OBSERVATIONS_INDEX_CHROMA", "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def capture_observation(data: dict) -> None:
    ...
    try:
        ...
        # El indexado vectorial carga dependencias pesadas. Mantenerlo opt-in
        # para que el hook PostToolUse no ralentice cada accion de Claude Code.
        if INDEX_CHROMA:
            try:
