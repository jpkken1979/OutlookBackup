...
def _extract_frontmatter_overrides(content: str) -> dict[str, str]:
    ...
    if isinstance(importance, str) and importance.strip().lower() in VALID_FRONTMATTER_IMPORTANCE:
...
def apply_changes(
    ...
    if result.rel_path not in existing:
        ...
        brain.ingest(
            ...
            # Titulos 100% japoneses no aportan palabras ASCII al slug;
            # el stem del archivo fuente es el fallback descriptivo.
            slug_hint=Path(result.rel_path).stem,
        )
        return "new"
...
def main() -> int:
    ...
    if not args.apply and (stats.to_ingest or stats.to_update or stats.archived_pending):
        print()
