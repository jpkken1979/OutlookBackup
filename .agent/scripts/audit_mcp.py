...
# Falsos positivos verificados (2026-05-30). Archivos cuyo PROPÓSITO es contener
# estos patrones: escáneres de seguridad que DEFINEN sus patrones de detección
# (eval/exec/os.system como strings/regex), un scorer con una lista de strings
# peligrosos para matching, y un generador de tests con credenciales dummy en sus
# plantillas (password="password"). No es un blanket-exclude: solo se silencia la
# categoría documentada por archivo; cualquier otra categoría sigue activa.
# Complementa el respeto a `# nosec` (los archivos con plantillas-string no pueden
# llevar el comentario inline). Las claves se comparan en formato POSIX.
FALSE_POSITIVE_ALLOWLIST: dict[str, set[str]] = {
    ".agent/agents/security-scanner/scripts/main.py": {"shell_injection"},
    ".agent/agents/security-auditor/scripts/security_scanner.py": {"shell_injection"},
    ".agent/agents/quality-scorer/scripts/quality_scorer.py": {"shell_injection"},
    ".agent/agents/code-reviewer/scripts/review_agent.py": {"shell_injection"},
    ".agent/agents/test-engineer/scripts/test_engineer.py": {"hardcoded_secret"},
    ".agent/agents/critic/scripts/critic.py": {"hardcoded_secret"},
}


def _is_false_positive(relative_path: str, category: str) -> bool:
    """Indica si (archivo, categoría) está en el allowlist de FPs verificados."""
    posix_path = relative_path.replace("\\", "/")
    return category in FALSE_POSITIVE_ALLOWLIST.get(posix_path, set())
...
def scan_file_for_patterns(
    ...
    try:
        ...
        # Falsos positivos verificados por archivo+categoría (scanners que definen
        # sus patrones, generadores de tests con credenciales dummy, etc.).
        if _is_false_positive(relative_path, category):
            return findings
        ...
        for line_num, line in enumerate(lines, start=1):
            # Respeta la convención `# nosec` (igual que bandit): si la línea la
            # lleva, es una supresión explícita revisada y no se reporta.
