...
# gitignore_guard: garantiza que el ecosistema no quede bloqueado por gitignore
# patterns en el target. Import diferido y opcional para no romper si el modulo
# core no esta disponible.
try:
    from core.gitignore_guard import patch_gitignore as _patch_gitignore

    GITIGNORE_GUARD_AVAILABLE = True
except ImportError:
    GITIGNORE_GUARD_AVAILABLE = False
...
def _configure_unicode_console() -> None:
    """Make the frozen CLI deterministic on non-UTF-8 Windows consoles."""
...
def _apply_gitignore_guard(target_dir: Path) -> None:
    """Patcha el .gitignore del target para no bloquear paths del ecosistema.

    Idempotente: si el bloque ya esta presente, no escribe. No bloquea el flujo
    si falla (solo emite warning).
    """
    if not GITIGNORE_GUARD_AVAILABLE:
        logger.debug("[gitignore_guard] modulo no disponible, skip")
        return
    try:
        result = _patch_gitignore(target_dir)
        if result.action == "unchanged":
            logger.info("[gitignore_guard] .gitignore ya esta protegido (no-op)")
        else:
            logger.info(
                "[gitignore_guard] %s: %s (%d paths protegidos)",
                result.action,
                result.path,
                result.paths_protected,
            )
    except Exception as exc:
        logger.warning("[gitignore_guard] no se pudo patchar .gitignore: %s", exc)
...
def write_json_file(path: Path, payload: dict[str, Any]) -> bool:
    ...
    from mcp_injector.io_utils import write_json_file as _impl

    return _impl(path, payload)
...
def merge_tree(
    ...
    """
    return _canonical_merge_tree(src, dst, exclude)
...
def get_mcp_servers(
    ...
    """Configuracion MCP del ecosistema. Implementacion canonica en el paquete.

    P4 dedup (2026-05-30): el cuerpo y los helpers de resolucion de interprete
    viven ahora en ``mcp_injector.mcp_config`` para evitar drift entre el
    monolito y el paquete modular. Este wrapper delega para mantener un unico
    origen de verdad (ver decision_mcp_injector_modularization.md).
    """
    from mcp_injector.mcp_config import get_mcp_servers as _impl
...
def safe_merge_json(file_path: Path, new_servers: dict[str, Any]) -> bool:
    """Mergea servidores MCP sobre un JSON basado en mcpServers. Delega al paquete.

    La implementacion canonica vive en ``mcp_injector.mcp_config`` (single source
    of truth): incluye el shallow-merge por server y la portabilizacion de paths
    (${USERPROFILE}/${ANTIGRAVITY_ROOT}) post-merge. Antes este monolito tenia su
    propia copia sin portabilizar, lo que reintroducia paths hardcoded de usuario
    al regenerar las configs (bugfix 2026-06-04).
    """
    from mcp_injector.mcp_config import safe_merge_json as _impl

    return _impl(file_path, new_servers)


def safe_merge_vscode_mcp(file_path: Path, new_servers: dict[str, Any]) -> bool:
    """Formato nativo de VS Code (clave "servers" + "type"). Delega al paquete.

    VS Code (GitHub Copilot) ignora el formato "mcpServers"; la implementacion
    canonica vive en ``mcp_injector.mcp_config`` (single source of truth).
    """
    from mcp_injector.mcp_config import safe_merge_vscode_mcp as _impl
...
def _read_ponytail_ruleset() -> str:
    """Lee el ruleset ponytail (lazy senior dev) desde la fuente compartida.

    Single source of verdad: `.claude/rules/ponytail.md`. Devuelve "" si falta,
    para degradar con gracia sin romper la inyeccion.
    """
    try:
        return (REPO_ROOT / ".claude" / "rules" / "ponytail.md").read_text(encoding="utf-8").strip()
    except OSError:
        return ""
...
def install_ai_manifest(
    ...
    ok = write_json_file(manifest_path, payload)
    if ok:
...
_HOOK_SCRIPT_RE = re.compile(r"((?:\.agent|\.claude)/[\w./-]+\.(?:py|sh))")


def _referenced_hook_scripts(command: str) -> set[str]:
    """Extrae las rutas relativas de scripts que un command de hook referencia.

    Resuelve el prefijo ``${CLAUDE_PROJECT_DIR}`` y el caso ``hook_runner.py <arg>.py``
    (hook_runner resuelve el arg relativo a su propia ubicacion: ``.agent/scripts/``).

    Args:
        command: El string del command del hook (desde settings.json).

    Returns:
        Conjunto de rutas relativas (POSIX) de scripts referenciados.
    """
    cmd = command.replace("${CLAUDE_PROJECT_DIR:-.}", "").replace("${CLAUDE_PROJECT_DIR}", "")
    scripts: set[str] = {m.lstrip("/") for m in _HOOK_SCRIPT_RE.findall(cmd)}
    runner = re.search(r"hook_runner\.py[\"']?\s+(\S+\.py)", cmd)
    if runner:
        scripts.add(f".agent/scripts/{runner.group(1)}")
    return scripts


def verify_and_heal_injected_hooks(target_dir: Path, repo_root: Path) -> list[str]:
    """Garantiza que todo hook cableado en el settings.json inyectado tenga su script.

    Root-cause guard (2026-07-10): el settings.json referencia scripts de hooks por
    path; si el injector no copio ese path (drift entre el settings y
    ``PORTABLE_AGENT_DIRS``/``CLAUDE_DIRS``), el hook falla con exit != 0 y Claude Code
    bloquea TODO prompt. Este check parsea el settings YA escrito y, por cada script
    referenciado que falte en el target, lo copia desde el repo fuente. Es auto-sanador
    y esta guiado por el contenido del settings, asi que cubre hooks futuros sin depender
    de actualizar una lista estatica.

    Args:
        target_dir: Directorio del proyecto destino ya inyectado.
        repo_root: Raiz del repo fuente (OpenAntigravity) de donde copiar.

    Returns:
        Lista de rutas relativas de scripts que se copiaron (healed).
    """
    ...
    try:
        ...
        logger.warning("[hooks-guard] No se pudo leer %s: %s", settings_path, exc)
        return []
            ...
            for group in groups:
                for hook in group.get("hooks", []):
                    ...
                    if isinstance(cmd, str):
                        referenced |= _referenced_hook_scripts(cmd)

    healed: list[str] = []
    for rel in sorted(referenced):
        if (target_dir / rel).exists():
            continue
        source_script = repo_root / rel
        if source_script.is_file():
            dst = target_dir / rel
            ensure_dir(dst.parent)
            if copy_file(source_script, dst):
                healed.append(rel)
                logger.info("[hooks-guard] Script de hook faltante copiado: %s", rel)
        else:
            logger.warning(
                "[hooks-guard] El settings referencia un hook inexistente tambien en el "
                "repo fuente: %s (revisar el template de settings.json)",
                rel,
            )
    if healed:
        logger.info("[hooks-guard] %d script(s) de hook auto-sanados", len(healed))
    return healed
...
def install_portable_runtime(
    ...
    from mcp_injector.bundle_v2 import install_from_source

    bundle_result = install_from_source(
        repo_root,
        target_dir,
        clients=[
            "claude",
    ...
    logger.info(
        "✅ [Bundle v2] %d archivo(s) modificados; digest=%s",
        bundle_result["change_count"],
        bundle_result["manifest"]["bundle_digest"][:12],
    )

    install_claude_settings(target_dir, repo_root)
    # Root-cause guard: garantiza que todo hook cableado en el settings tenga su
    # script en el target (evita el bug de UserPromptSubmit que bloquea prompts).
    verify_and_heal_injected_hooks(target_dir, repo_root)
...
def show_injection_diff(src_dir: Path, dst_dir: Path) -> None:
    ...
    for src_file in src_dir.rglob("*"):
        ...
        try:
            ...
            continue
        # Respetar SKIP_TREE_NAMES (mismo criterio que merge_tree/hash_tree) para no
        # reportar como "NUEVOS" artefactos transitorios como __pycache__/*.pyc que la
        # propia ejecucion de Python acaba de generar al importar modulos del target.
        # Sin este filtro, cada reinyeccion reportaba cientos de .pyc como novedad.
...
def install_full(
    ...
    _apply_gitignore_guard(target_dir)
...
def main() -> None:
    ...
    if is_bundle_command(sys.argv[1:]):
        raise SystemExit(
            run_bundle_cli(
                sys.argv[1:],
                default_repo_root=Path(__file__).parent.parent.parent,
            )
        )
    ...
    if args.target_dir:
        ...
        if not target_path.exists() or not target_path.is_dir():
            ...
            emit_json(
                {
                    "success": False,
                    "error": f"La ruta destino no existe o no es directorio: {target_path}",
                }
            )
            sys.exit(1)
        ...
        if args.update_hooks_only:
            ...
            _apply_gitignore_guard(target_path)
