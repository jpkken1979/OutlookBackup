...
def render_reference_markdown(entry: DesignReference) -> str:
    ...
    remote_excerpt = entry.remote_excerpt or "Remote excerpt unavailable. Use the style notes below."
    return "\n".join(
        [
            f"# {entry.name}",
            "",
...
def write_design_cache(entries: list[DesignReference], designs_dir: Path | None = None) -> list[Path]:
...
def sync_catalog(
    *,
    ...
    selected_entries = [entry for entry in entries if cache_slugs is None or entry.slug in cache_slugs]
