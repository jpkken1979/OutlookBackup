...
def _resolve_npx() -> str | None:
    """Locate npx across platforms.

    Falls back to known Windows install paths when PATH is stripped
    (Claude Code's shell sometimes inherits a minimal PATH that misses
    Machine-scope entries like ``C:\\Program Files\\nodejs``).

    Returns:
        Absolute path to npx executable, or None if not found.
    """
