...
def _resolves_to_sibling_module(py_file: Path, target: str) -> bool:
    """True si `target` es un modulo local hermano del archivo.

    Un script auto-contenido (tipico en skills empaquetados) puede tener su
    propio `core.py` o `core/` junto al script. En ese caso `from core import ...`
    resuelve a ese modulo local, NO al `core/` del runtime, y no debe contar como
    violacion de boundary.

    Args:
        py_file: Ruta absoluta del archivo que hace el import.
        target: Nombre del modulo importado (ej. "core").

    Returns:
        True si existe `<dir>/{target}.py` o `<dir>/{target}/__init__.py` junto al archivo.
    """
    parent = py_file.parent
    return (parent / f"{target}.py").is_file() or (parent / target / "__init__.py").is_file()
        ...
        for rule in BOUNDARY_RULES:
            ...
            for line_no, stmt, target in imports:
                if target in rule.forbidden_imports:
                    # Falso positivo: el target resuelve a un modulo hermano local
                    # del archivo (ej. skills/ui-ux-pro-max/scripts/core.py), que NO
                    # es el `core/` del runtime. Un skill auto-contenido puede traer
                    # su propio core.py; eso no viola R2.
                    if _resolves_to_sibling_module(py_file, target):
                        continue
