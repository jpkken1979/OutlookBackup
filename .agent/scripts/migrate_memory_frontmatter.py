...
def _derive_description(body: str, name: str) -> str:
    for line in body.splitlines():
        stripped = line.strip()
        if stripped and not stripped.startswith(("#", "---", "```", ">", "|")) and stripped != name:
            return stripped[:140]
...
def main() -> int:
    ...
    summary = {
        ...
        "skipped": len(skipped),
        "changes": [
            {"file": p.path.name, "added_keys": list(p.added_keys.keys())} for p in changed[:50]
        ],
    }
    ...
    if args.json:
        ...
        print(
            f"[{mode}] {len(changed)} archivo(s) a modificar, "
