...
def _install_session_hooks(target_dir: Path) -> None:
    ...
    already_installed = any(hook_cmd in str(h) for h in stop_hooks)
    ...
    if not already_installed:
        ...
        logger.info("  [session-hooks] installed Stop hook for session reporting")
    else:
        logger.info("  [session-hooks] already installed, skipping")
...
def inject_workspace(
    ...
    json_targets = [
        ...
        ("VS Code MCP", target_dir / ".vscode" / "mcp.json"),
