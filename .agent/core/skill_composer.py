...
class SkillComposer:
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            ...
            try:
                ...
                if registry.skills_dirs and Path(registry.skills_dirs[0]).resolve() != self.skills_path.resolve():
    ...
    async def validate_composition(self, skill: ComposedSkill) -> ValidationResult:
        ...
        # Estimate complexity
        complexity = "low"
        if len(skill.components) > 5:
            complexity = "high"
        elif len(skill.components) > 2:
            complexity = "medium"
    ...
    async def execute_composed(
        ...
        try:
            if skill.composition_type == CompositionType.SEQUENTIAL:
            ...
            elif skill.composition_type == CompositionType.PARALLEL:
                # Execute all at once
                tasks = [self._execute_component(c, input_data, ctx) for c in skill.components]
                component_results = await asyncio.gather(*tasks, return_exceptions=True)  # type: ignore[assignment]  # gather returns mixed types with return_exceptions

                # Check for failures
                for _i, result in enumerate(component_results):
                    if isinstance(result, Exception):
                        raise result

            elif skill.composition_type == CompositionType.PIPELINE:
                # Output feeds into next input
                for component in skill.components:
                    result = await self._execute_component(component, current_data, ctx)
                    component_results.append(result)
                    if not result.get("success", False):
            ...
            elif skill.composition_type == CompositionType.FALLBACK:
            ...
            elif skill.composition_type == CompositionType.CONDITIONAL:
