...
class OpenpyxlBackend:
    ...
    def create_chart(
        ...
        # Select chart class by type. LineChart no usa .type (no acepta col/bar).
        if chart_type == "line":
            chart = LineChart()
        else:
            chart = BarChart()
            ...
            if chart_type not in type_map:
                raise ValueError(
                    f"chart_type no soportado: {chart_type!r}. "
                    f"Validos: {sorted(set(type_map) | {'line'})}"
                )
            chart.type = type_map[chart_type]
        chart.add_data(data, titles_from_data=True)
        if title is not None:
            chart.title = title
        # openpyxl espera Legend|None, no boolean. None oculta la leyenda;
        # la default ya es visible asi que no hace falta tocar cuando legend=True.
        if not legend:
            chart.legend = None
        chart_id = id(chart)
    ...
    def conditional_format(
        ...
        if rule_type == "cell_value":
            # Si threshold es numerico, default operator 'greaterThan'.
            # Si es string, asumimos que es un operador valido openpyxl
            # (greaterThan, lessThan, equal, between, etc).
            if isinstance(formula_or_threshold, (int, float)):
                operator = "greaterThan"
                threshold = float(formula_or_threshold)
            else:
                operator = formula_or_threshold
                threshold = 0.0
            ws.conditional_formatting.add(
                range_addr,
                CellIsRule(operator=operator, formula=[str(threshold)], fill=fill, font=font),
            )
        elif rule_type == "expression":
            ws.conditional_formatting.add(
                range_addr,
                FormulaRule(formula=[str(formula_or_threshold)], fill=fill, font=font),
            )
        else:
            ws.conditional_formatting.add(
                range_addr,
