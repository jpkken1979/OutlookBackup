...
class SkillOrchestrator:
    ...
    @staticmethod
    def _extract_description_from_frontmatter(frontmatter_text: str) -> str | None:
        """Extrae el campo ``description`` de un frontmatter YAML.

        Intenta primero parsear el YAML y, si falla (caso típico cuando la
        descripción contiene dos puntos sin comillas como ``Triggers: foo``),
        cae a una extracción por regex sobre el texto crudo.

        Args:
            frontmatter_text: Texto del frontmatter YAML (sin los marcadores ---).

        Returns:
            La descripción como string, o ``None`` si no se encontró.
        """
    ...
    @staticmethod
    def _parse_triggers_from_description(description: str) -> list[str]:
        """Parsea la sección ``Triggers:`` de una descripción de skill.

        Busca una sección ``Triggers:`` y devuelve los keywords/frases
        separados por coma, normalizados (sin comillas ni espacios, en minúsculas).

        Args:
            description: Texto de la descripción del skill.

        Returns:
            Lista de triggers normalizados, o lista vacía si no hay sección.
        """
    ...
    def _extract_triggers_from_skill_md(skill_md_path: Path) -> list[str]:
        ...
        description = SkillOrchestrator._extract_description_from_frontmatter(fm_match.group(1))
        if not description:
            return []

        return SkillOrchestrator._parse_triggers_from_description(description)
