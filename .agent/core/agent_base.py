...
class AntigravityAgent(ABC):
    ...
    async def _apply_pre_reasoning(self, task: str, execution_context: dict) -> None:
        """Ejecuta el pre-razonamiento y enriquece el contexto de ejecución.

        Args:
            task: Tarea a ejecutar.
            execution_context: Contexto de ejecución a mutar in-place con el
                resultado del razonamiento (si lo hubiera).
        """
    ...
    async def _apply_reflection(
        ...
        """Reflexiona sobre el output, adjunta metadata y decide si reintentar.

        Args:
            task: Tarea ejecutada.
            result: Resultado de la ejecución (se muta in-place si es dict).
            execution_context: Contexto de ejecución (recibe retry_hints si aplica).
            attempts: Número de intento actual.
            max_retries: Máximo de reintentos permitidos.

        Returns:
            True si la reflexión sugiere reintentar y aún quedan intentos.
        """
        ...
        if reflection_result.should_retry and attempts <= max_retries:
            logger.info(f"Reflexión sugiere reintentar (intento {attempts}/{max_retries + 1})")
            execution_context["retry_hints"] = reflection_result.retry_suggestions
            return True
    ...
    async def _try_recover(
        ...
        """Intenta recuperación inteligente tras un error de ejecución.

        Args:
            error: Excepción capturada durante la ejecución.
            task: Tarea ejecutada.
            execution_context: Contexto de ejecución (recibe recovery_hints si aplica).
            attempts: Número de intento actual.
            max_retries: Máximo de reintentos permitidos.

        Returns:
            True si la recuperación fue exitosa y se debe reintentar.
        """
        if self.intelligence and attempts <= max_retries:
            recovery = await self.recover_from_error(
                error, {"task": task, "context": execution_context}
            )
    ...
    async def run(
        ...
        if use_reasoning and self.intelligence:
            await self._apply_pre_reasoning(task, execution_context)
            ...
            while attempts <= max_retries:
                ...
                try:
                    ...
                    if use_reflection and self.intelligence and success:
                        should_retry = await self._apply_reflection(
                            task, result, execution_context, attempts, max_retries
                        )
                        if should_retry:
                            continue
                    ...
                    if await self._try_recover(e, task, execution_context, attempts, max_retries):
                        continue
