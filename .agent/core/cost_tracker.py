...
class CostTracker:
    ...
    def __init__(self, storage_path: str | None = None):
        ...
        elif os.environ.get("ANTIGRAVITY_COST_TRACKING_PATH"):
            self.storage_path = Path(os.environ["ANTIGRAVITY_COST_TRACKING_PATH"])
        else:
            self.storage_path = Path.home() / ".antigravity" / "data" / "cost_tracking.json"
    ...
    @staticmethod
    def _group_records(
        records: list[UsageRecord], key: Callable[[UsageRecord], str]
    ) -> dict[str, dict]:
        """Agrupa registros por una clave acumulando tokens, costo y llamadas.

        Args:
            records: Registros de uso ya filtrados.
            key: Función que extrae la clave de agrupación de cada registro.

        Returns:
            Diccionario clave -> {"tokens", "cost", "calls"} acumulados.
        """
        grouped: dict[str, dict] = {}
        for r in records:
            k = key(r)
            if k not in grouped:
                grouped[k] = {"tokens": 0, "cost": 0, "calls": 0}
            grouped[k]["tokens"] += r.total_tokens
            grouped[k]["cost"] += r.estimated_cost
            grouped[k]["calls"] += 1
        return grouped
    ...
    def _build_recommendations(
        ...
        Args:
            filtered: Registros considerados en el reporte.
            by_agent: Costos agrupados por agente.
            by_model: Costos agrupados por modelo.
            total_cost: Costo total del período.
    ...
    def get_report(
        ...
        # Group by agent / model / task type
        by_agent = self._group_records(filtered, lambda r: r.agent)
        by_model = self._group_records(filtered, lambda r: r.model)
        by_task_type = self._group_records(filtered, lambda r: r.task_type or "unknown")
        ...
        recommendations = self._build_recommendations(filtered, by_agent, by_model, total_cost)
