...
class MetaOrchestrator:
    ...
    @staticmethod
    def _score_to_quality_level(overall_score: float) -> QualityLevel:
        """Mapea un score global [0,1] a un nivel de calidad.

        Args:
            overall_score: Score ponderado de la ejecución.

        Returns:
            QualityLevel correspondiente al umbral alcanzado.
        """
    ...
    def _analyze_strengths(
        success_rate: float, duration_efficiency: float, avg_quality: float
    ) -> list[str]:
        """Determina las fortalezas de la ejecución según las métricas."""
        strengths: list[str] = []
        if success_rate >= 0.9:
    ...
    @staticmethod
    def _analyze_weaknesses(results: list[AgentResult], duration_efficiency: float) -> list[str]:
        """Determina las debilidades de la ejecución (fallos, lentitud)."""
        weaknesses: list[str] = []
    ...
    async def evaluate_execution(
        ...
        strengths = self._analyze_strengths(success_rate, duration_efficiency, avg_quality)
        weaknesses = self._analyze_weaknesses(results, duration_efficiency)
