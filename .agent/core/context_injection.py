...
def _serialize_prompt_section(
    ...
    *,
    ...
    if context.project_stack:
        sections.append(f"**Stack:** {context.project_stack}")

    if context.project_dir:
        sections.append(f"**Directorio:** {context.project_dir}")

    if context.current_file:
        sections.append(f"**Archivo actual:** {context.current_file}")

    if context.open_files:
        files_str = ", ".join(context.open_files)
        sections.append(f"**Archivos abiertos:** {files_str}")

    if context.recent_edits:
        edits_str = ", ".join(context.recent_edits)
        sections.append(f"**Ediciones recientes:** {edits_str}")

    if context.recent_errors:
        sections.append("**Errores recientes:**")
        for err in context.recent_errors:
            sections.append(f"- {err}")

    if context.relevant_decisions:
        sections.append("**Decisiones relevantes:**")
        for dec in context.relevant_decisions:
            sections.append(f"- {dec}")

    if context.memory_hints:
        sections.append("**Conocimiento previo:**")
