...
class SelfImprover:
    ...
    def analyze_recent_executions(self, days: int = 7) -> list[PerformanceInsight]:
        ...
        for agent_name, agent_execs in by_agent.items():
            insights.extend(self._agent_insights(agent_name, agent_execs))
    ...
    def _agent_insights(
        self, agent_name: str, agent_execs: list[ExecutionMetrics]
    ) -> list[PerformanceInsight]:
        """Genera insights de rendimiento para un agente.

        Args:
            agent_name: Nombre del agente.
            agent_execs: Ejecuciones del agente en la ventana analizada.

        Returns:
            Lista de PerformanceInsight (success_rate, quality, retries, error recurrente).
        """
