...
class LangGraphEngine:
    ...
    async def _run_graph(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        while iterations < max_iter:
            ...
            try:
                if status == "planning":
                    updates = await planner_node(state, self._config)
                elif status == "executing":
                    updates = await executor_node(state, self._config)
                elif status == "critiquing":
                    updates = await critic_node(state, self._config)
                elif status == "routing":
                    updates = await router_node(state, self._config)
                else:
