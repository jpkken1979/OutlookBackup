...
class ExecutionEngine:
    ...
    async def execute_agent(self, agent_name: str, task: str) -> AgentResult:
        ...
        if not script_path:
            result = self._build_guidance_result(agent_name, task)
            if self.on_agent_complete:
        ...
        try:
            ...
            result = await self._finalize_script_result(
                agent_name=agent_name,
                task=task,
                proc=proc,
                output=output,
            ...
            result = await self._handle_agent_exception(
                agent_name=agent_name, task=task, exc=e, start_time=start_time
            )
    ...
    def _build_guidance_result(self, agent_name: str, task: str) -> AgentResult:
        """Construye el resultado en modo guia cuando el agente no tiene script.

        Args:
            agent_name: Nombre del agente.
            task: Tarea solicitada.

        Returns:
            AgentResult SUCCESS con el contenido de IDENTITY.md si existe, o
            SKIPPED si el agente no tiene ni script ni IDENTITY.md.
        """
    ...
    async def _finalize_script_result(
        self,
        *,
        agent_name: str,
        task: str,
        ...
        """Procesa el resultado de un script: contexto, buses, costo y AgentResult.

        Args:
            agent_name: Nombre del agente ejecutado.
            task: Tarea ejecutada.
            proc: Proceso ya finalizado (se usa returncode).
            output: Salida decodificada del proceso.
            stderr: Salida de error cruda del proceso.
            execution_time: Tiempo de ejecucion en milisegundos.

        Returns:
            AgentResult con el estado segun el returncode del proceso.
        """
        ...
        if self.memory_bus and succeeded:
        ...
        if self.message_bus and succeeded:
            with contextlib.suppress(Exception):
        ...
        return AgentResult(
            ...
            output=output,
            error=stderr.decode("utf-8", errors="replace") if not succeeded else None,
            execution_time_ms=execution_time,
    ...
    async def _handle_agent_exception(
        ...
        *,
        ...
        """Construye el AgentResult de fallo, consulta recovery y avisa al bus.

        Args:
            agent_name: Nombre del agente que fallo.
            task: Tarea en ejecucion al momento del fallo.
            exc: Excepcion capturada.
            start_time: Marca de inicio para calcular el tiempo transcurrido.

        Returns:
            AgentResult con status FAILED y, si aplica, metadata de recovery.
        """
        error_str = f"Error: {str(exc)}\n{traceback.format_exc()}"
        recovery_info = await self._lookup_recovery_info(exc)
    ...
    async def _lookup_recovery_info(self, exc: Exception) -> dict | None:
        """Consulta el subsistema de recovery para una excepcion dada.

        Args:
            exc: Excepcion a analizar.

        Returns:
            Diccionario con la estrategia de recovery encontrada, o None si no
            hay recovery disponible o no se hallo estrategia.
        """
        ...
        try:
            ...
            if recovery_result and recovery_result.found:
                return {
                    "strategy": recovery_result.strategy.name if recovery_result.strategy else None,
    ...
    def _build_agent_system_prompt(self, agent_dir: "Path") -> str:
        """Construye el system prompt concatenando los archivos de identidad del agente.

        Args:
            agent_dir: Directorio del agente que contiene IDENTITY.md / SYSTEM_PROMPT.md.

        Returns:
            System prompt combinado (cadena vacía si no hay archivos presentes).
        """
        parts = []
    ...
    def _build_previous_context(self) -> "str | None":
        """Construye el bloque de contexto con los outputs de agentes previos.

        Returns:
            Cadena con los resultados previos, o None si no hay contexto útil.
        """
        ...
        for prev_agent, prev_data in self._context.items():
            if isinstance(prev_data, dict) and prev_data.get("output"):
                context_parts.append(f"## Results from {prev_agent}:\n{prev_data['output'][:1000]}")
    ...
    async def _try_autonomous_execution(
        ...
        try:
            ...
            system_prompt = self._build_agent_system_prompt(agent_dir)

            # Include context from previous agents
            context_str = self._build_previous_context()
