...
def _search_knowledge_files(
    ...
    *,
    ...
    for project_name, project_data in projects.items():
        ...
        for filename, _file_meta in files.items():
            ...
            if any(kw in content_lower for kw in ("error", "fix", "bug", "issue", "resolved")):
                insight_type = "error_solution"
            elif any(
                kw in content_lower for kw in ("decision", "decided", "approach", "architecture")
            ):
                insight_type = "decision"
            elif any(kw in content_lower for kw in ("skill", "agent", "tool")):
                insight_type = "skill_used"
