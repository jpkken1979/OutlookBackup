...
class DynamicTeamOrchestrator:
    ...
    async def create_team(
        ...
        if required_domains:
            domain_agents = self.find_agents_for_domains(required_domains)
            for agent in domain_agents:
                if agent not in candidates:
                    candidates.append(agent)

        # Add from task config preferred agents
        ...
        for _i, agent in enumerate(selected):
            role = roles_needed.pop(0) if roles_needed else TeamRole.SUPPORTER

            member = TeamMember(agent_name=agent.agent_name, role=role, capabilities=agent)
