...
def _expand_env_headers(headers: dict[str, str]) -> dict[str, str]:
    """Expande variables ${VAR} en los valores de headers usando os.environ.

    Args:
        headers: Headers con posibles placeholders ${VAR}.

    Returns:
        Headers con las variables de entorno resueltas (vacío si no existen).
    """
    expanded: dict[str, str] = {}
    for key, value in headers.items():
        if isinstance(value, str) and "${" in value:
            expanded[key] = re.sub(r"\$\{(\w+)\}", lambda m: os.environ.get(m.group(1), ""), value)
        else:
            expanded[key] = value
    return expanded


def _classify_url_error_reason(reason: str) -> str:
    """Traduce el motivo de un URLError a un mensaje de error legible.

    Args:
        reason: Texto del motivo del URLError.

    Returns:
        Mensaje de error descriptivo según el patrón detectado.
    """
    if "host_not_allowed" in reason or "403" in reason:
        return f"Blocked by proxy: {reason}"
    if "timed out" in reason:
        return f"Connection timeout ({HEALTH_CHECK_TIMEOUT}s)"
...
class MCPConnectionMonitor:
    ...
    async def _check_http_server(
        ...
        expanded = _expand_env_headers(headers)
        ...
        try:
            ...
            latency = (time.monotonic() - start) * 1000
            reason = str(e.reason)
            error_msg = _classify_url_error_reason(reason)
