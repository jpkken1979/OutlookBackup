...
class PluginRegistry:
    ...
    def _passes_filters(
        ...
        """Determina si una entrada satisface los filtros de búsqueda.

        Args:
            entry: Entrada del registro a evaluar.
            category: Filtro por categoría (None = sin filtro).
            tag: Filtro por tag (None = sin filtro).
            min_version: Versión mínima requerida (None = sin filtro).

        Returns:
            True si la entrada pasa todos los filtros aplicados.
        """
    ...
    @staticmethod
    def _score_term_in_collection(
        term: str, items: list[str], weight: float, label: str
    ) -> tuple[float, str | None]:
        """Puntúa la primera coincidencia de un término dentro de una colección.

        Args:
            term: Término de búsqueda en minúsculas.
            items: Colección de strings a inspeccionar (tags, skills, capabilities).
            weight: Puntaje a otorgar si hay coincidencia.
            label: Etiqueta para la razón de match (ej. "tag", "skill").

        Returns:
            Tupla (puntaje, razón) — (0.0, None) si no hay coincidencia.
        """
        for item in items:
            if term in item.lower():
                return weight, f"{label}: {item}"
        return 0.0, None

    @staticmethod
    def _score_term(entry: "RegistryEntry", term: str) -> tuple[float, list[str]]:
        """Calcula el aporte de relevancia de un término sobre una entrada.

        Args:
            entry: Entrada del registro a puntuar.
            term: Término de búsqueda en minúsculas.

        Returns:
            Tupla (puntaje acumulado del término, razones de match).
        """
        score = 0.0
        reasons: list[str] = []

        # Name match (exact or contains)
        ...
        # Collection matches (tags, skills, capabilities)
        for items, weight, label in (
            (entry.tags, 4.0, "tag"),
            (entry.skills, 3.0, "skill"),
            (entry.capabilities, 2.0, "capability"),
        ):
            col_score, col_reason = PluginRegistry._score_term_in_collection(
                term, items, weight, label
            )
    ...
    def search(
        ...
        for entry in self._index.entries.values():
            if not self._passes_filters(entry, category, tag, min_version):
                continue

            score = 0.0
            reasons: list[str] = []
            for term in query_terms:
                term_score, term_reasons = self._score_term(entry, term)
                score += term_score
