...
async def main() -> None:
    ...
    async with MCPClientManager() as manager:
        if args.list_servers:
            ...
            tools = await manager.list_tools(args.list_tools)
            print(f"Herramientas de {args.list_tools} ({len(tools)}):")
            for tool in tools:
            ...
            server, tool_name, args_json = args.call
            ...
            result = await tools.context7_get_docs(args.context7)  # type: ignore[attr-defined]  # dynamic method via MCP
            if result.success:
                print(f"Documentación de {args.context7}:")
