...
class _ActiveWatch:
    ...
    finished_stdout: threading.Event = field(default_factory=threading.Event)
    finished_stderr: threading.Event = field(default_factory=threading.Event)
...
class ProcessWatcher:
    ...
    def _validate_regex_safety(self, regex: str, label: str) -> None:
        if len(regex) > self._MAX_REGEX_LEN:
            raise WatcherError(f"regex en pattern {label!r} excede {self._MAX_REGEX_LEN} chars")
    ...
    def tail(self, watch_id: str, lines: int = 50, stream: str = "both") -> dict[str, Any]:
        ...
        if active is not None:
            # Wait for the relevant stream(s) to fully drain. Waiting on the
            # aggregate finished_streaming would return as soon as the first
            # stream finished, possibly truncating the other one.
            if stream in ("stdout", "both"):
                active.finished_stdout.wait(timeout=2.0)
            if stream in ("stderr", "both"):
                active.finished_stderr.wait(timeout=2.0)
            with active.lock:
                if stream in ("stdout", "both"):
                    stdout_tail = [
    ...
    def matches(self, watch_id: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT * FROM matches WHERE watch_id = ? ORDER BY matched_at DESC LIMIT ?",
    ...
    def _wait_exit(self, active: _ActiveWatch) -> None:
        ...
        try:
            ...
            exit_code = -1
        # Drain both stream readers before persisting so match_count reflects
        # matches that arrive after proc.wait() returns but before pipe
        # buffers are fully consumed. Without this, the persisted match_count
        # can be stale and a fresh watcher reading the DB will see 0 matches
        # even though w1 (in memory) sees 1.
        active.finished_stdout.wait(timeout=2.0)
        active.finished_stderr.wait(timeout=2.0)
        with active.lock:
            if active.state == "running":
    ...
    def _check_patterns(
        ...
        scan_text = (
            line if len(line) <= self._MAX_LINE_FOR_REGEX else line[: self._MAX_LINE_FOR_REGEX]
        )
    ...
    def _active_to_dict(
        ...
        if include_tail:
            out["stdout_tail"] = [{"line": n, "text": t} for n, t in list(active.tail_stdout)[-20:]]
            out["stderr_tail"] = [{"line": n, "text": t} for n, t in list(active.tail_stderr)[-20:]]
        return out
