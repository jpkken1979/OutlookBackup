...
class RedisMessageBus:
    ...
    async def _subscribe_redis(
        ...
        """Itera mensajes de un canal vía Redis Consumer Groups.

        Crea el consumer group si no existe y entrega cada entrada del stream
        como un ``BusMessage``, reintentando ante errores transitorios.

        Args:
            channel: Nombre lógico del canal a suscribir.
            consumer: Nombre único del consumidor dentro del grupo.
            group: Nombre del consumer group.
            block_ms: Tiempo de bloqueo en milisegundos por lectura.

        Yields:
            Cada mensaje recibido del stream Redis.
        """
        stream_key = f"antigravity:stream:{channel}"
    ...
    async def _subscribe_memory(
        ...
        """Itera mensajes de un canal vía la cola en memoria (fallback).

        Args:
            channel: Nombre lógico del canal a suscribir.
            block_ms: Timeout en milisegundos por intento de lectura.

        Yields:
            Cada mensaje recibido de la cola en memoria.
        """
    ...
    async def subscribe(
        ...
        if self._use_redis:
            async for msg in self._subscribe_redis(channel, consumer, group, block_ms):
                yield msg
        else:
            async for msg in self._subscribe_memory(channel, block_ms):
                yield msg
    ...
    def _resolve_pending_reply(self, corr_id: "str | None", payload: Any) -> None:
        """Resuelve el Future pendiente asociado a un correlation_id.

        Args:
            corr_id: Identificador de correlacion de la respuesta.
            payload: Carga util a entregar al Future.
        """
    ...
    async def _reply_listener_redis(self) -> None:
        """Escucha replies A2A sobre Redis Pub/Sub (canales a2a:reply:*)."""
        pubsub = self._backend.pubsub()
        ...
        try:
            while self._running:
                ...
                if raw and raw.get("type") == "pmessage":
                    try:
                        data = json.loads(raw["data"])
                        self._resolve_pending_reply(
                            data.get("correlation_id"), data.get("payload", {})
                        )
                    except Exception as e:
    ...
    async def _reply_listener_sqlite(self) -> None:
        """Escucha replies A2A sobre el fallback SQLite via polling de cola."""
        queue = await self._backend.subscribe_queue("a2a:reply:*")
        try:
            while self._running:
                try:
                    msg = await asyncio.wait_for(queue.get(), timeout=2.0)
                    self._resolve_pending_reply(msg.correlation_id, msg.payload)
                except TimeoutError:
                    continue
        except asyncio.CancelledError:
            return
    ...
    async def _start_reply_listener(self) -> None:
        ...
        """
        if self._use_redis:
            await self._reply_listener_redis()
        else:
            await self._reply_listener_sqlite()
