...
from core.portable_runtime import (
    PORTABLE_SCRIPT_FLAG,
    is_portable_runtime,
    project_root,
...
class GatewayExecutor:
    ...
    def __init__(self, orchestrator: Any = None) -> None:
        self._portable_mode = is_portable_runtime() and orchestrator is None
        if orchestrator is not None:
            self._orch = orchestrator
            self.orchestrator = orchestrator
        elif self._portable_mode:
            self._orch = None
    ...
    @staticmethod
    def _portable_agents() -> list[dict[str, Any]]:
        """Discover agents from the project bundle without loading the orchestrator."""

        agents: list[dict[str, Any]] = []
        agents_root = project_root() / ".agent" / "agents"
            ...
            agents.append(
                {
                    ...
                    "has_executable": resolve_agent_script(agent_dir.name) is not None,
                    "identity_path": str(agent_dir / "IDENTITY.md"),
                    "agent_dir": str(agent_dir),
                    "status": "ready",
                }
            )
    ...
    async def _execute_portable_agent(
        ...
        """Execute one bundled agent with the frozen interpreter."""

        started = time.monotonic()
        script = resolve_agent_script(agent_name)
        if script is None:
            result = ExecutionResult(
                success=False,
                agent=agent_name,
                task=task,
                error="Portable agent script was not found",
                execution_time_ms=(time.monotonic() - started) * 1000,
                tier="portable",
                metadata={"execution_mode": "portable-script"},
            )
            self._record_portable_result(result)
            return result
        ...
        process = await asyncio.create_subprocess_exec(
            ...
            env={
                **os.environ,
                "ANTIGRAVITY_AGENT_INPUT": task,
                "ANTIGRAVITY_HOME": str(project_root()),
            },
        )
        try:
            ...
            result = ExecutionResult(
                ...
                agent=agent_name,
                task=task,
                error="Portable agent execution timed out",
                execution_time_ms=(time.monotonic() - started) * 1000,
                tier="portable",
                metadata={"execution_mode": "portable-script"},
            )
        ...
        result = ExecutionResult(
            ...
            agent=agent_name,
            task=task,
            result=output[:10_000] if success else "",
            error=error[:2_000] if not success else "",
            execution_time_ms=(time.monotonic() - started) * 1000,
            tier="portable",
            metadata={
                "execution_mode": "portable-script",
    ...
    def _record_portable_result(self, result: ExecutionResult) -> None:
        """Record one portable execution without loading shared global services."""

        self._history.append({**result.to_dict(), "timestamp": time.time()})
        if len(self._history) > 500:
            self._history = self._history[-500:]
        self.execution_history = self._history
        self.cost_tracker.record_cost(result.agent, cost=0.0)
    ...
    def _execute_agent_inner(
        ...
        try:
            ...
            if agent_name == "local-executor":
                ...
                try:
                    result_data = loop.run_until_complete(self._orch.execute(task_description=task))
                finally:
                    loop.close()
                tier = (
                    result_data.get("tier", "unknown")
                    if isinstance(result_data, dict)
                    else "unknown"
                )
    ...
    @staticmethod
    def _parse_local_executor_task(task: str) -> "tuple[str | None, str | None]":
        """Parsea la task en (cmd, script_path_arg) segun el prefijo de flag.

        Args:
            task: Descripcion de la task, opcionalmente prefijada con --command o --script.

        Returns:
            Tupla (cmd, script_path_arg). Solo uno de los dos es no-None segun el caso.
        """
        if task.startswith("--command "):
            return task[len("--command ") :].strip(), None
        if task.startswith("--script "):
            return None, task[len("--script ") :].strip()
        if task.startswith("--"):
    ...
    @staticmethod
    def _extract_stdout_marker_output(stdout: str) -> str:
        """Extrae la salida real luego del marker STDOUT: emitido por local_executor.

        Args:
            stdout: Salida cruda capturada del proceso.

        Returns:
            La porcion de salida posterior al marker, o el stdout original si no existe.
        """
        if "STDOUT:" not in stdout:
            return stdout
    ...
    def _run_local_executor_command(self, cmd: str, script_path: "Path") -> dict:
        """Ejecuta un comando via cmd.exe builtin o via el script local_executor.

        Args:
            cmd: Comando a ejecutar (string sin parsear).
            script_path: Ruta al script local_executor.py.

        Returns:
            Dict con success/result/output o success/error/output.
        """
        import subprocess
        import shlex
        import sys
        ...
        use_cmd = bool(cmd_list) and cmd_list[0].lower() in NEED_CMD
        try:
            if use_cmd:
                # Wrap builtin commands with cmd.exe /c.
                # SECURITY: pasamos los tokens ya parseados por shlex (cmd_list),
                # NO la string cruda. Con `cmd /c <string>`, cmd.exe reinterpreta
                # metacaracteres (&, |, >, &&) habilitando command injection aun
                # con shell=False. Pasar la lista evita esa reinterpretación: cada
                # token va como argumento separado del builtin.
                proc = subprocess.run(
                    ["cmd", "/c", *cmd_list],
                    capture_output=True,
                    text=True,
                ...
                output = self._extract_stdout_marker_output(stdout)
                if "STDOUT:" not in stdout and proc.returncode != 0 and stderr:
                    output = stderr
    ...
    @staticmethod
    def _run_local_executor_info(script_path: "Path") -> dict:
        """Ejecuta el local_executor en modo --info (interactive/info mode).

        Args:
            script_path: Ruta al script local_executor.py.

        Returns:
            Dict con success/result/output o success/error.
        """
        import subprocess
        import sys
    ...
    def _execute_local_executor(self, task: str) -> dict:
        ...
        cmd, script_path_arg = self._parse_local_executor_task(task)

        if script_path_arg:
            # --script: ejecuta script via subprocess con cmd.exe /c
            return self._execute_local_executor_script(script_path_arg)
        if cmd:
            return self._run_local_executor_command(cmd, script_path)
        # Interactive or info mode
        return self._run_local_executor_info(script_path)
