...
class LearningLoop:
    ...
    def __init__(self, project_root: str | None = None):
        ...
        elif os.environ.get("ANTIGRAVITY_LEARNING_ROOT"):
            self.project_root = Path(os.environ["ANTIGRAVITY_LEARNING_ROOT"]).resolve()
        else:
            self.project_root = Path.home() / ".antigravity"
    ...
    def record_action(self, action: str, context: dict | None = None) -> str:
        ...
        """
        # Sufijo uuid: garantiza unicidad aunque datetime.now() colisione en el mismo
        # microsegundo (o este congelado en tests), evitando event_ids duplicados que
        # hacen que _find_event devuelva siempre el primer evento y se pisen los outcomes.
        event_id = f"evt_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{uuid.uuid4().hex[:8]}"
...
def get_learning_loop(project_root: str | None = None) -> LearningLoop:
    """Obtiene instancia singleton.

    Args:
        project_root: Raiz explicita para el estado de aprendizaje. ``None``
            delega en la resolucion 3-tier de ``LearningLoop`` (env
            ``ANTIGRAVITY_LEARNING_ROOT`` o ``~/.antigravity``) en vez de
            escribir ``.agent/memory/learning`` en el cwd del proceso.
    """
