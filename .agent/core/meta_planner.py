...
class MetaPlanner:
    ...
    # --------------------------------------------------------
    @staticmethod
    def _detect_domains(task_lower: str) -> dict[str, float]:
        """Detecta dominios por keywords y los puntúa (matches / total keywords).

        Args:
            task_lower: Tarea en minúsculas.

        Returns:
            Mapa dominio -> score normalizado (solo dominios con al menos 1 match).
        """
        detected: dict[str, float] = {}
        for domain, keywords in DOMAIN_KEYWORDS.items():
            score = sum(1.0 for kw in keywords if kw in task_lower)
            if score > 0:
                detected[domain] = score / len(keywords)
        return detected

    @staticmethod
    def _classify_complexity(n_domains: int) -> str:
        """Clasifica la complejidad de la tarea según la cantidad de dominios."""
        if n_domains >= 4:
            return "expert"
        if n_domains >= 3:
            return "complex"
    ...
    def analyze(self, task: str) -> dict[str, Any]:
        ...
        detected_domains = self._detect_domains(task_lower)
        ...
        complexity = self._classify_complexity(len(sorted_domains))
