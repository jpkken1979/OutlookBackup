...
def analyze_project_gaps(
    ...
    *,
    ...
    for rule in GAP_RULES:
        ...
        if "deps" in detect:
            if deps & set(detect["deps"]):
                matched = True

        # Check deps_file existence
        if "deps_file" in detect:
            if (target / detect["deps_file"]).exists():
                matched = True
        ...
        already_exists = any(missing_key in skill_name for skill_name in existing_skills)
        if already_exists:
            continue

        # Create suggestion with project name interpolated
        template = rule["skill"]
    ...
    for rule in AGENT_RULES:
        if _check_condition(rule["condition"], target, deps):
            ...
            report.agent_suggestions.append(agent_suggestion)
