...
class AdaptiveTopology:
    ...
    def detect_clusters(self, min_size: int = 2) -> list[dict[str, Any]]:
        ...
        for agent in agents_by_degree:
            ...
            cluster_members = [agent]
            visited.add(agent)

            # Agregar vecinos con alta afinidad
            for target, edge in node.edges.items():
                if target in visited:
                    continue
                if edge.weight >= CLUSTER_AFFINITY_MIN:
                    cluster_members.append(target)
                    visited.add(target)

            if len(cluster_members) >= min_size:
                # Calcular cohesión (promedio de pesos entre miembros)
                total_weight = 0.0
                pair_count = 0
                ...
                cohesion = total_weight / pair_count if pair_count > 0 else 0

                # Dominio principal del cluster
