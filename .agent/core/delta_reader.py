...
# Columna de conteo por modo de lectura. Whitelist literal interno: NUNCA recibe
# input de usuario. Cualquier valor fuera de este dict no genera SQL.
_STATS_MODE_COLUMNS: dict[str, str] = {
    "full": "reads_full",
    "delta": "reads_delta",
...
# Plantilla de upsert de stats. El placeholder __COL__ se reemplaza (str.replace
# sobre literales, sin f-string/format/%) por una columna del whitelist de arriba,
# por lo que el SQL queda estatico y sin vector de inyeccion. Los datos van como
# parametros (?). Se precomputa por modo al importar el modulo.
_STATS_UPSERT_TEMPLATE = (
    "INSERT INTO stats("
    "session_id, reads_total, __COL__, bytes_full, bytes_served, created_at"
    ") VALUES (?, 1, 1, ?, ?, ?) "
...
_STATS_UPSERT_BY_MODE: dict[str, str] = {
    mode: _STATS_UPSERT_TEMPLATE.replace("__COL__", column)
    for mode, column in _STATS_MODE_COLUMNS.items()
}
    ...
    def _persist_and_track(
        ...
        with self._connect() as conn:
            ...
            try:
                ...
                # SQL estático por modo: el nombre de columna NUNCA se interpola/concatena
                # (cualquier formateo dispararía B608 y, aunque el valor venga de un
                # whitelist interno, preferimos statements literales sin vector posible).
                # Solo los valores van como parámetros (?).
                stmt = _STATS_UPSERT_BY_MODE.get(result.mode)
                if stmt is not None:
                    conn.execute(stmt, (session_id, size, bytes_served, now))
                conn.execute("COMMIT;")
            except Exception:
