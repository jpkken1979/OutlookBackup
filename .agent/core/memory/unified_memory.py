...
class UnifiedMemory:
    ...
    def shared_memory(self):
        ...
        ``shared_memory.json``), que es la interfaz que los call-sites de ``store()``
        necesitan (líneas con ``self.shared_memory.set(...)``).

        Antes intentaba cargar ``core.shared_memory.SharedMemory`` primero, pero esa
        clase expone ``.store()``/``.retrieve()`` y NO ``.set()``: cuando el import
        tenía éxito (lo habitual), ``store(MemoryType.CONTEXT)`` rompía con
        ``AttributeError: 'SharedMemory' object has no attribute 'set'``. Bug latente
        documentado en ``.claude/memory/decision_agentmemory_migration_spike.md``
        (spike 019).
        """
    ...
    async def _store_entity(self, content: str, agent: str, metadata: dict | None) -> str:
        """Almacena una entidad en el temporal graph (con import fallback de EntityType).

        Args:
            content: Nombre/contenido de la entidad.
            agent: Agente origen.
            metadata: Metadatos (puede contener entity_type).

        Returns:
            ID de la entidad agregada.
        """
        try:
        ...
        entity_type = (
            metadata.get("entity_type", EntityType.CONCEPT) if metadata else EntityType.CONCEPT
        )
    ...
    async def store(
        ...
        if memory_type == MemoryType.OBSERVATION:
            ...
            if self.temporal_graph:
                return await self._store_entity(content, agent, metadata)
    ...
    async def query(
        ...
        if self.dual_stream and (
            ...
            results.extend(await self._query_dual_stream(query, limit, agent_filter, task_filter))

        # Query TemporalGraph
        if self.temporal_graph and MemoryType.ENTITY in memory_types:
            results.extend(await self._query_temporal_graph(query, limit, agent_filter))

        # Query ephemeral
        if include_ephemeral and MemoryType.EPHEMERAL in memory_types:
            results.extend(self._query_ephemeral(query))

        # Sort by score
        results.sort(key=lambda x: x.score, reverse=True)
        return results[:limit]

    async def _query_dual_stream(
        self,
        query: str,
        limit: int,
        ...
        """Consulta el subsistema DualStream y mapea sus items a resultados.

        Args:
            query: Texto de busqueda.
            limit: Cantidad maxima de items a recuperar.
            agent_filter: Filtro opcional por agente.
            task_filter: Filtro opcional por tarea.

        Returns:
            Lista de UnifiedMemoryResult (vacia si falla la consulta).
        """
        results: list[UnifiedMemoryResult] = []
        try:
    ...
    async def _query_temporal_graph(
        self,
        query: str,
        limit: int,
        agent_filter: "str | None",
    ) -> "list[UnifiedMemoryResult]":
        """Consulta el TemporalGraph y mapea entidades a resultados.

        Args:
            query: Texto de busqueda.
            limit: Cantidad maxima de entidades a recuperar.
            agent_filter: Filtro opcional por agente.
    ...
    def _query_ephemeral(self, query: str) -> "list[UnifiedMemoryResult]":
        """Filtra la memoria efimera por coincidencia de substring.

        Args:
            query: Texto de busqueda (case-insensitive).

        Returns:
            Lista de UnifiedMemoryResult de los items efimeros que matchean.
        """
        results: list[UnifiedMemoryResult] = []
...
class SimpleSharedMemory:
    ...
    def store(self, memory_type: str, data: dict[str, Any], agent: str | None = None) -> None:
        """Compatibilidad con ``SharedMemory`` para consumidores de decisiones."""
        entry = {"data": data, "agent": agent, "timestamp": datetime.now().isoformat()}
        if memory_type in {"decision", "learning"}:
            key = f"{memory_type}s"
            entries = self._data.get(key, [])
            if not isinstance(entries, list):
                entries = []
            entries.append(entry)
            self.set(key, entries[-(100 if memory_type == "decision" else 50) :])
            return
        if memory_type == "context":
            contexts = self._data.get("context", {})
            if not isinstance(contexts, dict):
                contexts = {}
            contexts[data.get("key", "default")] = entry
            self.set("context", contexts)
            return
