...
class GeminiClient(BaseLLMClient):
    ...
    @staticmethod
    def _convert_content_item_to_part(item: object) -> dict | None:
        """Convierte un item de contenido (texto/tool_result/tool_use) a un part Gemini.

        Args:
            item: Item de la lista de contenido (dict tipado o valor simple).

        Returns:
            El part Gemini equivalente, o None si el dict no tiene un type soportado.
        """
        ...
        if item_type == "tool_use":
            return {"function_call": {"name": item["name"], "args": item.get("input", {})}}
        return None
    ...
    def _convert_messages_to_gemini(
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                parts = [
                    part
                    for part in (self._convert_content_item_to_part(item) for item in content)
                    if part is not None
                ]
                if parts:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        data = await self._post_with_retry(httpx, url, request_body)

        latency_ms = (time.time() - start_time) * 1000

        # Extract content and tool calls
        candidates = data.get("candidates", [])
        content, tool_calls = self._extract_content_and_tool_calls(candidates)
    ...
    async def _post_with_retry(self, httpx, url: str, request_body: dict) -> dict:
        ...
        Args:
            httpx: Modulo httpx ya importado por el llamador.
            url: Endpoint completo de generateContent con la API key.
            request_body: Cuerpo JSON de la request.

        Returns:
            Respuesta JSON decodificada de Gemini.

        Raises:
            Exception: El ultimo error capturado si todos los intentos fallan.
        """
        last_error = None
    ...
    @staticmethod
    def _extract_content_and_tool_calls(candidates: list[dict]) -> tuple[str, list[dict]]:
        """Extrae el texto y las llamadas a herramientas de los candidates.

        Args:
            candidates: Lista de candidates devuelta por Gemini.

        Returns:
            Tupla (content, tool_calls) con el texto concatenado y los tool calls.
        """
        content = ""
