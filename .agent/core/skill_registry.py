    ...
    def _search_keyword(self, query: str, k: int) -> list[tuple[str, float]]:
        ...
        for skill_name, lazy_record in self._lazy_records.items():
            ...
            if skill_name == query_lower:
                score += 1.0
            # Name prefix match
            elif skill_name.startswith(query_lower):
                score += 0.8
            ...
            # Tags match
            tags = lazy_record.frontmatter.get("tags", [])
            ...
            # Triggers match
            triggers = lazy_record.frontmatter.get("triggers", [])
