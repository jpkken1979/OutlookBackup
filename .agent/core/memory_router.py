    ...
    async def preload_context(self, task_description: str, *, limit: int = 5) -> ContextBundle:
        ...
        if pm:
            try:
                ...
                logger.debug("Failed to load known errors for context bundle", exc_info=True)

            try:
                decisions = pm.search_decisions(task_description)
                bundle.recent_decisions = [{"decision": str(d)} for d in decisions[:limit]]
            except Exception:
                logger.debug("Failed to load decisions for context bundle", exc_info=True)
            ...
            try:
                ...
                logger.debug("Failed to load project health for context bundle", exc_info=True)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if brain:
            try:
                ...
                logger.debug("Failed to retrieve brain network stats", exc_info=True)
        ...
        if pm:
            try:
                ...
                logger.debug("Failed to retrieve project health stats", exc_info=True)
