...
class RateTier(str, Enum):
    ...
    Attributes:
        ...
        LOCAL: Loopback clients — burst protection but no daily cap.
...
DEFAULT_TIER_CONFIGS: dict[RateTier, TierConfig] = {
    ...
    # El gateway bindea 127.0.0.1: Nexus, Claude Code (MCP), el CLI y el bot
    # comparten TODOS el mismo client_id "127.0.0.1", asi que un tope diario por
    # IP es un balde compartido — el cliente mas charlatan deja sin servicio a los
    # demas. Paso real (2026-07-27): el polling de Nexus agoto los 5000/dia de
    # STANDARD en 4.4h y el broker MCP empezo a devolver BROKER_ERROR.
    # Se conserva el limite por minuto (protege contra bucles descontrolados);
    # se elimina solo el tope diario, que no protege de nada en loopback.
    RateTier.LOCAL: TierConfig(
        name=RateTier.LOCAL,
        lifetime_limit=0,
        per_minute_limit=300,
        per_day_limit=0,
