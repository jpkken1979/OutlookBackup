...
class ConsensusProtocol:
    ...
    def _evaluate_consensus(
        ...
        if proposal.mode == ConsensusMode.QUORUM:
            return self._evaluate_quorum(proposal, len(active_votes), winner, winner_pct)
        return self._evaluate_threshold_mode(proposal.mode, winner, winner_pct)

    @staticmethod
    def _evaluate_quorum(
        proposal: Proposal, vote_count: int, winner: str | None, winner_pct: float
    ) -> tuple[bool, str]:
        """Evalúa consenso en modo QUORUM (quorum mínimo + mayoría simple)."""
        if vote_count < proposal.quorum_minimum:
            return False, f"Quorum no alcanzado: {vote_count}/{proposal.quorum_minimum} votantes"
        if winner_pct > 0.5:
            return True, (
    ...
    @staticmethod
    def _evaluate_threshold_mode(
        mode: ConsensusMode, winner: str | None, winner_pct: float
    ) -> tuple[bool, str]:
        """Evalúa consenso en modos basados en umbral de porcentaje del ganador."""
        ...
        if mode == ConsensusMode.SUPERMAJORITY:
            if winner_pct > 0.666:
        ...
        if mode == ConsensusMode.WEIGHTED:
            # La confianza ponderada del ganador debe ser > 50%
            if winner_pct > 0.5:
                return True, (
