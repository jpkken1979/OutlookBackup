...
class UserPreferenceModel:
    ...
    def __init__(self, storage_path: str | None = None):
        # Resolution: explicit param > env var > user home (NO repo path)
        # Bugfix 2026-05-23: el default antiguo (.agent/memory/user_model.json) ensuciaba
        # el working tree cuando tests cargaban claude_core (ver
        # .claude/memory/bugfix_user_model_json_dirty_after_tests.md)
        if storage_path:
            self.storage_path = Path(storage_path)
        elif os.environ.get("ANTIGRAVITY_USER_MODEL_PATH"):
            self.storage_path = Path(os.environ["ANTIGRAVITY_USER_MODEL_PATH"])
        else:
            self.storage_path = Path.home() / ".antigravity" / "memory" / "user_model.json"
