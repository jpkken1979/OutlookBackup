...
"""
    ...
    def _eval_completeness(
        task_lower: str, output: str, output_lower: str, score: float
    ) -> tuple[float, str, list[str], list[str]]:
        """Evalúa la dimensión COMPLETENESS de forma heurística.

        Args:
            task_lower: Tarea en minúsculas.
            output: Salida original del agente.
            output_lower: Salida en minúsculas.
            score: Puntaje base a ajustar.

        Returns:
            Tupla (score, reasoning, concerns, suggestions).
        """
        concerns: list[str] = []
        ...
        return score, f"Keyword coverage: {coverage:.0%}", concerns, suggestions

    @staticmethod
    def _eval_correctness(
        output_lower: str, score: float
    ) -> tuple[float, str, list[str], list[str]]:
        """Evalúa la dimensión CORRECTNESS de forma heurística.
        ...
        return score, f"Uncertainty markers: {uncertainty_count}", concerns, []

    @staticmethod
    def _eval_clarity(output: str, score: float) -> tuple[float, str, list[str], list[str]]:
        """Evalúa la dimensión CLARITY de forma heurística.
        ...
        reasoning = f"Has structure: {has_structure}, Avg sentence length: {avg_sentence_len:.0f}"
        return score, reasoning, concerns, suggestions

    @staticmethod
    def _eval_actionability(
        output: str, output_lower: str, score: float
    ) -> tuple[float, str, list[str], list[str]]:
        """Evalúa la dimensión ACTIONABILITY de forma heurística.
        ...
        return score, f"Has actions: {has_actions}, Has code: {has_code}", [], suggestions

    @staticmethod
    def _eval_safety(output_lower: str, score: float) -> tuple[float, str, list[str], list[str]]:
        """Evalúa la dimensión SAFETY de forma heurística.
        ...
        if dangers_found:
            # Penalización proporcional: un solo patrón resta 0.3 (compat hacia
            # atrás), cada patrón adicional agrava el riesgo (+0.15), con tope en
            # 0.7. Acumular varias señales de peligro NO debe diluirse en el
            # promedio ponderado — un output con rm -rf + drop table + eval( debe
            # caer mucho más que uno con un único marcador.
            penalty = min(0.3 + 0.15 * (len(dangers_found) - 1), 0.7)
            score -= penalty
            concerns.extend([f"Potentially dangerous pattern: {d}" for d in dangers_found])
            suggestions.append("Review security implications")

        return score, f"Danger patterns found: {len(dangers_found)}", concerns, suggestions
    ...
    def _heuristic_evaluate_dimension(
        ...
        base_score = 0.7  # Default moderate confidence
        output_lower = output.lower()
        task_lower = task.lower()

        if dimension == ReflectionDimension.COMPLETENESS:
            score, reasoning, concerns, suggestions = self._eval_completeness(
                task_lower, output, output_lower, base_score
            )
        elif dimension == ReflectionDimension.CORRECTNESS:
            score, reasoning, concerns, suggestions = self._eval_correctness(
                output_lower, base_score
            )
        elif dimension == ReflectionDimension.CLARITY:
            score, reasoning, concerns, suggestions = self._eval_clarity(output, base_score)
        elif dimension == ReflectionDimension.ACTIONABILITY:
            score, reasoning, concerns, suggestions = self._eval_actionability(
                output, output_lower, base_score
            )
        elif dimension == ReflectionDimension.SAFETY:
            score, reasoning, concerns, suggestions = self._eval_safety(output_lower, base_score)
            ...
            concerns = []
            suggestions = ["Verify alignment with prior decisions"]
        else:
            score, reasoning, concerns, suggestions = base_score, "", [], []
