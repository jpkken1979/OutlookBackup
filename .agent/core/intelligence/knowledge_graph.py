...
class KnowledgeGraph:
    ...
    def __init__(
        ...
        elif os.environ.get("ANTIGRAVITY_KNOWLEDGE_GRAPH_PATH"):
            self.storage_path = Path(os.environ["ANTIGRAVITY_KNOWLEDGE_GRAPH_PATH"])
        else:
            self.storage_path = Path.home() / ".antigravity" / "memory" / "knowledge_graph"
    ...
    def get_related(
        ...
        acc = (related_nodes, related_edges, relevance_scores)
        ...
        for depth in range(max_depth):
            ...
            for current_id in current_level:
                next_level.extend(
                    self._expand_neighbors(current_id, edge_types, visited, depth, acc)
                )
    ...
    def _expand_neighbors(
        ...
        visited: set[str],
        depth: int,
        acc: tuple[list, list, dict],
    ) -> list[str]:
        """Expande vecinos (salientes + entrantes) de un nodo en el BFS.

        Muta `visited` y los acumuladores en `acc` in-place. Agrega cada vecino
        nuevo una sola vez con el primer edge cuyo tipo matchee.

        Args:
            current_id: Nodo actual.
            edge_types: Tipos de edge a seguir (None = todos).
            visited: Conjunto de nodos ya vistos (se muta).
            depth: Profundidad actual (para el decay de relevancia).
            acc: Tupla (related_nodes, related_edges, relevance_scores) a poblar.

        Returns:
            Lista de IDs de vecinos nuevos descubiertos en este paso.
        """
        related_nodes, related_edges, relevance_scores = acc
        new_ids: list[str] = []
        # (other_id, edge_key) para salientes y entrantes
        pairs = [(t, (current_id, t)) for t in self._adjacency[current_id]]
        pairs += [(s, (s, current_id)) for s in self._reverse_adjacency[current_id]]

        for other_id, edge_key in pairs:
            if other_id in visited:
                continue
            for edge in self._edge_index[edge_key]:
                if edge_types is None or edge.edge_type in edge_types:
                    ...
                    relevance_scores[other_id] = 1.0 / (depth + 2)
                    break
        return new_ids
    ...
    def remove_node(self, node_id: str) -> bool:
        ...
        # Update adjacency. Usar pop(..., None): si el nodo nunca tuvo edges
        # salientes/entrantes, el defaultdict no creó la clave y un `del` directo
        # lanzaría KeyError al remover un nodo aislado.
