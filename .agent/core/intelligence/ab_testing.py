...
class ABTester:
    ...
    def __init__(self, memory_dir: Path | None = None):
        self.memory_dir = memory_dir or Path(".agent/memory/ab_testing")
    ...
    async def compare(
        ...
        metrics_breakdown: dict[str, dict[str, float]] = {agent: {} for agent in agents}
        ...
        for output in outputs:
            for metric_name in metrics:
                metric_type = (
                    ...
                    else MetricType.QUALITY
                )
                score = self.evaluator.evaluate(output, task, metric_type)
                output.metrics[metric_name] = score

                if metric_name not in metrics_breakdown[output.agent_name]:
                    metrics_breakdown[output.agent_name][metric_name] = []
                if isinstance(metrics_breakdown[output.agent_name][metric_name], list):
                    metrics_breakdown[output.agent_name][metric_name].append(score)

        # Average scores
        for agent in agents:
            for metric in metrics:
                if isinstance(metrics_breakdown[agent].get(metric), list):
                    scores = metrics_breakdown[agent][metric]
                    metrics_breakdown[agent][metric] = statistics.mean(scores) if scores else 0.0
