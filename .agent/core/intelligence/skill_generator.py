...
class SkillGenerator:
    ...
    def __init__(self, memory_dir: Path | None = None, skills_dir: Path | None = None):
        self.memory_dir = self._resolve_dir(
            memory_dir, "ANTIGRAVITY_SKILL_GEN_MEMORY_DIR", ".agent/memory/skill_generator"
        )
        self.skills_dir = self._resolve_dir(
            skills_dir, "ANTIGRAVITY_SKILL_GEN_SKILLS_DIR", ".agent/skills"
        )
    ...
    @staticmethod
    def _resolve_dir(explicit: Path | None, env_var: str, default: str) -> Path:
        """Resuelve un directorio con precedencia: param explícito > env var > default.

        Permite a los tests redirigir la escritura del generador a un directorio
        temporal vía env var (ANTIGRAVITY_SKILL_GEN_*), evitando contaminar el
        .agent/ real del repositorio (test pollution). En producción, sin la env
        var seteada, conserva el default histórico.

        Args:
            explicit: Directorio pasado explícitamente al constructor (gana siempre).
            env_var: Nombre de la variable de entorno a consultar como override.
            default: Ruta por defecto si no hay param ni env var.

        Returns:
            El Path resuelto según la precedencia descrita.
        """
    ...
    async def record_execution(self, execution: dict[str, Any]) -> None:
        ...
        rotate_jsonl_if_needed(history_file, max_lines=5000)
