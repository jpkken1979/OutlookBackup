...
class DocumentationParser:
    ...
    def parse_readme(self, content: str, tool_name: str) -> dict[str, Any]:
        ...
        if paragraphs:
            # Skip headers, find first real paragraph
            for p in paragraphs:
                clean = p.strip()
        ...
        # Extract parameters from options sections
        ...
        if options_match:
            ...
            # Parse option lines like: -v, --verbose  Description here
        ...
        for pattern in warning_patterns:
            ...
            for match in matches[:3]:
                result["gotchas"].append(match.strip()[:200])
...
class ToolLearner:
    ...
    async def suggest_usage(
        ...
        if best_example:
            ...
            # Build generic command from parameters
            command = tool_name
            for param in knowledge.parameters[:3]:
                if param.required:
                    command += f" {param.name}"
            explanation = "Generic usage based on required parameters"
            confidence = 0.4

        # Find alternatives from related tools
        alternatives = []
