...
is_within_root = _security_utils.is_within_root
...
class AgentToolManager:
    ...
    def _tool_write_file(self, path: str, content: str) -> str:
        ...
        if not _is_path_in_writable_roots(file_path, writable_roots):
            ...
            raise PermissionError(f"Path outside WRITABLE_ROOTS: {path!r} (allowed: {roots_str})")
    ...
    def _tool_search_codebase(self, pattern: str, path: str = ".") -> list[dict]:
        ...
        try:
            ...
            output = self._search_codebase_fallback(pattern, path)

        return self._parse_grep_output(output)

    def _search_codebase_fallback(self, pattern: str, path: str) -> str:
        """Busca el patron leyendo archivos cuando grep no esta disponible.

        Args:
            pattern: Expresion regular ya validada a buscar en cada linea.
            path: Directorio raiz desde donde recorrer recursivamente.

        Returns:
            Cadena con lineas en formato "filepath:linenum:content" (max 50).
        """
    ...
    def _parse_grep_output(output: str) -> list[dict]:
        """Convierte la salida estilo grep en una lista de coincidencias.

        Args:
            output: Texto con lineas en formato "filepath:linenum:content".

        Returns:
            Lista de dicts con claves file/line/content (max 50 resultados).
        """
