...
class _DpApi:
    ...
    def encrypt(self, plaintext: bytes) -> bytes | None:
        ...
        data_in = CRYPT_INTEGER_BLOB(len(plaintext), ctypes.cast(ctypes.create_string_buffer(plaintext), ctypes.c_void_p))
    ...
    def decrypt(self, ciphertext: bytes) -> bytes | None:
        ...
        data_in = CRYPT_INTEGER_BLOB(len(ciphertext), ctypes.cast(ctypes.create_string_buffer(ciphertext), ctypes.c_void_p))
        out_buf = ctypes.create_string_buffer(65536)
        data_out = CRYPT_INTEGER_BLOB(65536, ctypes.cast(out_buf, ctypes.c_void_p))
...
def _derive_fernet_key(key_path: Path) -> bytes:
    ...
    Uses HKDF-like stretch: base64(sha256(b"dpapi-fallback" + str(key_path).encode()))
    truncated and formatted as a Fernet key (URL-safe base64 of 32-byte key + sig).
    """
    import hashlib
    import hmac

    secret = b"antigravity-session-key-v1" + str(key_path).encode()
    raw = hashlib.sha256(secret).digest()  # 32 bytes
    # Fernet key is 32 bytes (encryption) + 32 bytes (sig) base64'd
    sig_key = hmac.new(b"fernet-signing-key-v1", raw, hashlib.sha256).digest()
    fernet_key = raw + sig_key
...
def encrypt_for_user(plaintext: bytes, key_path: Path) -> bytes:
    ...
    if _is_windows:
        ...
        logger.warning("DPAPI encrypt failed (ret=%s, len=%d), falling back to Fernet",
                       result is not None, len(result) if result else 0)
