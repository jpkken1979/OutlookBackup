...
class Mem0Adapter:
    ...
    def __init__(self, gateway_url: str = "http://127.0.0.1:4747") -> None:
    ...
    async def search(self, query: str, limit: int = 10) -> list[KnowledgeEntry]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                async with session.get(f"{self.gateway_url}/v1/mem0/recall", params=params) as resp:
                    if resp.status != 200:
                        return []
    ...
    async def store(self, content: str, metadata: dict[str, Any] | None = None) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                async with session.post(f"{self.gateway_url}/v1/mem0/store", json=payload) as resp:
    ...
    def _read_note(self, path: Path) -> KnowledgeEntry | None:
        ...
        try:
            ...
            return KnowledgeEntry(
                id=f"vault_{hashlib.md5(rel_path.encode()).hexdigest()[:12]}",
        ...
        try:
            ...
            async with client:
                ...
                for i, note in enumerate(notes[:limit]):
                    entry = KnowledgeEntry(
                        id=f"obsidian_{hashlib.md5(note.path.encode()).hexdigest()[:12]}",
...
class UnifiedKnowledgeBridge:
    ...
    async def store(
        ...
        if KnowledgeSource.OBSIDIAN in targets:
            if category == KnowledgeCategory.DECISION:
                ...
                results["obsidian"] = path is not None
            elif category == KnowledgeCategory.SESSION:
                ...
                results["obsidian"] = False  # Solo decisions y sessions por ahora
        ...
        if KnowledgeSource.PROJECT in targets:
            ...
            if pm and category == KnowledgeCategory.DECISION:
                try:
                    ...
                    results["project_memory"] = True
                except Exception:
                    results["project_memory"] = False
            elif pm and category == KnowledgeCategory.ERROR:
                try:
                    pm.record_error(
                        pattern=content,
    ...
    async def generate_knowledge_brief(self, max_chars: int = 8000) -> str:
        ...
        if pk.decisions:
            ...
            for d in pk.decisions[:10]:
                sections.append(f"- **{d.timestamp}**: {d.content[:200]}")

        # Errores conocidos y resoluciones
        if pk.errors:
            sections.append("\n## Errores Conocidos y Resoluciones\n")
            for e in pk.errors[:10]:
                sections.append(f"- {e.content[:200]}")

        # Hotspots
        if pk.hotspots:
            sections.append("\n## Archivos Hotspot (más modificados)\n")
            for h in pk.hotspots[:8]:
                sections.append(f"- {h.content}")

        # Sesiones recientes
        if pk.sessions:
            sections.append("\n## Sesiones Recientes\n")
            for s in pk.sessions[:5]:
                sections.append(f"- **{s.timestamp}**: {s.content[:200]}")
        ...
        if pk.ci_health:
            ...
            sections.append(f"- Estado: **{status}** (éxito: {rate:.0%})")

        # Test trends
        if pk.test_trends:
            latest = pk.test_trends[-1] if pk.test_trends else {}
            if latest:
                sections.append("\n## Tests\n")
                sections.append(
        ...
        # Obsidian notes
        if pk.obsidian_notes:
            sections.append("\n## Notas de Obsidian\n")
            for n in pk.obsidian_notes[:5]:
                path = n.metadata.get("path", "")
                sections.append(f"- [{path}] {n.content[:150]}")

        # mem0 memories
        if pk.mem0_memories:
            sections.append("\n## Memorias Semánticas (mem0)\n")
            for m in pk.mem0_memories[:5]:
...
def _deduplicate(entries: list[KnowledgeEntry]) -> list[KnowledgeEntry]:
    ...
    for entry in entries:
        content_hash = hashlib.md5(entry.content[:200].lower().encode()).hexdigest()
