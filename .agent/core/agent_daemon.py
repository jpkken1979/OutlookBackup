...
class AgentDaemon(DaemonPlanningMixin, DaemonCoordinationMixin, DaemonAdvancedMixin):
    ...
    async def _stop_subsystems(self) -> None:
        """Detiene los subsistemas async para evitar tareas huérfanas.

        Para AgentObservatory y ReactiveEventSystem si existen y exponen un
        método ``stop``. Cualquier error se loguea en nivel debug sin propagar.
        """
    ...
    async def _drain_and_cancel_workers(self, drain_timeout: float) -> None:
        """Drena las tareas activas y luego cancela workers y heartbeat.

        Espera hasta ``drain_timeout`` segundos a que terminen las tareas
        activas, después cancela las tareas worker y el heartbeat y aguarda
        su finalización.

        Args:
            drain_timeout: Segundos máximos a esperar el drenado de tareas activas.
        """
    ...
    async def stop(self, drain_timeout: float = 30.0) -> None:
        ...
        await self._drain_and_cancel_workers(drain_timeout)
