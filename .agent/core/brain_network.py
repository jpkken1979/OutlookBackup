...
from .brain import Brain, BrainNode, LintReport
...
class BrainNetwork:
    ...
    def promote_concepts(self) -> list[BrainNode]:
        ...
        for app_id, brain in self._app_brains.items():
            nodes = brain.list_nodes(status="active")
            for node in nodes:
                for tag in node.tags:
                    tag_apps.setdefault(tag, set()).add(app_id)

        # Incluir tags del Mother Brain
        mother_nodes = self.mother.list_nodes(status="active")
        ...
        for tag, apps in tag_apps.items():
            if (
                ...
                # Recolectar contexto de todos los nodos con este tag
                context_parts: list[str] = []
                related_slugs: list[str] = []

                for app_id, brain in self._app_brains.items():
                    nodes = brain.list_nodes(tag=tag, status="active")
                    for node in nodes:
                        context_parts.append(f"**{app_id}**: {node.title} ({node.date})")
                        related_slugs.append(node.slug)
