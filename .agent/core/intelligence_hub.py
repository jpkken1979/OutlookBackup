...
class IntelligenceHub:
    ...
    def _apply_memory_replay(
        self,
        task: str,
        tech_stack: list[str] | None,
        ctx: IntelligenceContext,
    ) -> None:
        """Enriquece el contexto con recalls del Memory Replay.

        Args:
            task: Descripcion de la tarea.
            tech_stack: Stack tecnologico relevante.
            ctx: Contexto de inteligencia a mutar in-place.
        """
        memory_replay = self._get_memory_replay()
        if not memory_replay:
            return
        try:
    ...
    def _apply_learning_loop(self, task: str, ctx: IntelligenceContext) -> None:
        """Enriquece el contexto con avoidances y correcciones del Learning Loop.
    ...
    def _apply_project_memory(self, task: str, ctx: IntelligenceContext) -> None:
        """Enriquece el contexto con datos del Project Memory.

        Incorpora errores conocidos, decisiones previas y ajusta el risk level
        segun el estado de salud del CI.
    ...
    def rank_agents(
        ...
        ranked: list[RankedAgent] = [
            self._score_candidate(candidate, task_type, task_lower, now) for candidate in candidates
        ]
    ...
    def _score_candidate(
        ...
        """Calcula el RankedAgent de un candidato combinando los 5 factores.

        Args:
            candidate: Dict con al menos 'name' y opcionalmente 'tier',
                'skills', 'description'.
            task_type: Tipo de tarea clasificado por classify_task.
            task_lower: Descripcion de la tarea en minusculas.
            now: Momento actual para calcular el bonus de recencia.

        Returns:
            RankedAgent con score acotado a [0, 1] y razones acumuladas.
        """
        ...
        skill_match = self._compute_skill_match(task_lower, skills, description, reasons)
        ...
        recency_bonus = self._compute_recency_bonus(perf, name, now, reasons)
        score += recency_bonus

        # --- Factor 5: Error penalty (0 to -0.15) ---
        score -= self._compute_error_penalty(perf, reasons)
    ...
    def _compute_skill_match(
        ...
        """Calcula la afinidad entre la tarea y los skills/descripcion del agente.

        Args:
            task_lower: Descripcion de la tarea en minusculas.
            skills: Lista de skills declarados por el agente.
            description: Descripcion textual del agente.
            reasons: Lista mutable donde se acumulan las razones del score.

        Returns:
            Afinidad en el rango [0, 1].
        """
        skill_match = 0.0
    ...
    def _compute_recency_bonus(
        ...
        """Calcula el bonus por uso reciente del agente.

        Args:
            perf: Registro de performance del agente o None.
            name: Nombre del agente (para logging de fechas invalidas).
            now: Momento actual.
            reasons: Lista mutable donde se acumulan las razones del score.

        Returns:
            Bonus de recencia en el rango [0, 0.10].
        """
        recency_bonus = 0.0
        if perf and perf.last_used:
            try:
                ...
                logger.debug("Invalid last_used date for agent %s: %r", name, perf.last_used)
        return recency_bonus

    @staticmethod
    def _compute_error_penalty(perf: Any, reasons: list[str]) -> float:
        """Calcula la penalizacion por errores historicos del agente.
        ...
        Returns:
            Penalizacion en el rango [0, 0.15] (se resta del score).
        """
        if not (perf and perf.error_types):
            return 0.0
    ...
    def _update_agent_performance(
        ...
        """Actualiza las metricas de performance acumuladas de un agente.

        Args:
            agent_name: Nombre del agente ejecutado.
            task_type: Tipo de tarea clasificada.
            success: Si la ejecucion fue exitosa.
            quality_score: Puntaje de calidad de la ejecucion.
            duration_ms: Duracion de la ejecucion en milisegundos.
            error_type: Tipo de error si lo hubo.
            now_str: Timestamp ISO de la ejecucion.
        """
    ...
    def _update_skill_effectiveness(
        ...
        """Actualiza la efectividad acumulada de los skills usados.

        Args:
            skills_used: Lista de skills empleados en la ejecucion.
            task_type: Tipo de tarea clasificada.
            success: Si la ejecucion fue exitosa.
            quality_score: Puntaje de calidad de la ejecucion.
            user_rating: Rating del usuario si lo hubo.
            now_str: Timestamp ISO de la ejecucion.
        """
    ...
    def _feed_learning_loop(
        ...
        """Registra accion y resultado en el Learning Loop.
        ...
        Args:
            ...
            task_type: Tipo de tarea clasificada.
            success: Si la ejecucion fue exitosa.
            corrections: Correcciones aplicadas si las hubo.
        """
        learning_loop = self._get_learning_loop()
        if not learning_loop:
            return
    ...
    def _feed_self_improver(
        ...
        """Registra la ejecucion en el Self-Improver.
    ...
    def _feed_metrics_collector(
        ...
        """Registra la invocacion y errores en el Metrics Collector.
    ...
    def _feed_project_memory(
        self,
        agent_name: str,
        error_type: str | None,
        corrections: str | None,
    ) -> None:
        """Registra errores recurrentes en el Project Memory.
        ...
        """
        project_memory = self._get_project_memory()
        if not (project_memory and error_type):
            return
        try:
    ...
    def record_outcome(
        ...
        self._update_agent_performance(
            agent_name, task_type, success, quality_score, duration_ms, error_type, now_str
        )

        # 2. Actualizar skill effectiveness
        self._update_skill_effectiveness(
            skills_used, task_type, success, quality_score, user_rating, now_str
        )

        # 3. Alimentar Learning Loop
        self._feed_learning_loop(task, agent_name, task_type, success, corrections)

        # 4. Alimentar Self-Improver
        self._feed_self_improver(
            agent_name, task_type, success, duration_ms, quality_score, error_type
        )

        # 5. Alimentar Metrics Collector
        self._feed_metrics_collector(task, agent_name, success, duration_ms, error_type)

        # 6. Alimentar Project Memory (errores recurrentes)
        self._feed_project_memory(agent_name, error_type, corrections)
