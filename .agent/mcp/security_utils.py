...
FUENTE CANÓNICA: Este archivo es la única fuente de verdad.
`mcp-server/security_utils.py` debe mantenerse sincronizado con este archivo.
Para sincronizar: copiar el contenido completo de este archivo al otro.
"""
...
def _origin_host_scheme_match(parsed_origin: SplitResult, allowed: str) -> bool:
    """Compara un origen ya parseado contra un origen permitido (host+scheme, puerto libre).

    Args:
        parsed_origin: Resultado de urlsplit() del origen entrante.
        allowed: Origen permitido en formato string.

    Returns:
        True si coinciden scheme y hostname y el permitido no fija puerto.
    """
    try:
...
def is_origin_allowed(origin: str, allowed_origins: list[str]) -> bool:
    ...
    return any(_origin_host_scheme_match(parsed_origin, allowed) for allowed in allowed_origins)
