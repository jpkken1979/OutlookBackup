...
from core.brain import scrub_surrogates  # noqa: E402
...
def handle_brain_register(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # VALIDACION CRITICA: brain_dir debe quedar dentro del project root
    # Previene path traversal via brain_register (CRIT-02 de auditoriajp 2026-05-13)
    brain_path = brain_path.resolve()
    project_root = network.project_root.resolve()
    if not str(brain_path).startswith(str(project_root)):
        return {
            "error": (
                f"brain_dir debe estar dentro del proyecto. "
                f"Recibido: {brain_dir!r}, proyecto: {project_root}"
            )
        }
        ...
        return ok(
            {
                "content": [
                    {
                        "type": "text",
                        "text": json.dumps(scrub_surrogates(result), ensure_ascii=False),
                    },
                ],
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if response is not None:
                print(json.dumps(scrub_surrogates(response), ensure_ascii=False), flush=True)
