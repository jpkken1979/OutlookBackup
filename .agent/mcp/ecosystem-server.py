...
def handle_connection_health(_args: dict[str, Any]) -> str:
    ...
    for name, config in servers.items():
        ...
        if transport in ("http", "url") and url:
            ...
            if not valid:
                results.append(f"🚫 {name} ({transport}): SSRF bloqueado - {reason}")
                continue
            try:
                ...
                results.append(f"✅ {name} ({transport}): HTTP {resp.status} - healthy")
            except urllib.error.HTTPError as e:
                results.append(f"⚠️ {name} ({transport}): HTTP {e.code} - {e.reason}")
            except urllib.error.URLError as e:
                reason = str(e.reason)
                if "host_not_allowed" in reason or "403" in reason:
                    results.append(f"🚫 {name} ({transport}): Blocked by proxy")
                elif "timed out" in reason:
                    results.append(f"⏱️ {name} ({transport}): Connection timeout")
                else:
                    results.append(f"❌ {name} ({transport}): {reason}")
            except Exception as e:
                results.append(f"❌ {name} ({transport}): {e}")
        else:
            # Verificar servidor stdio (solo que el script existe)
            cmd = config.get("command", "")
            args = config.get("args", [])
            if cmd in ("python", "python3") and args:
                script = BASE_DIR / args[0]
                if script.exists():
                    results.append(f"✅ {name} (stdio): Script exists")
                else:
                    results.append(f"❌ {name} (stdio): Script not found: {args[0]}")
            elif cmd:
                import shutil

                if shutil.which(cmd):
                    results.append(f"✅ {name} (stdio): Command available")
                else:
                    results.append(f"❌ {name} (stdio): Command not found: {cmd}")
        ...
        for fname in pm.get("files", {}):
            ...
            for line_num, line in enumerate(content.splitlines(), 1):
                if query in line.lower():
                    results.append(f"[{project_name}/{fname}:{line_num}] {line.strip()}")
