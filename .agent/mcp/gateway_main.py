...
try:
    from .gateway_middleware import RateLimiter, TTLCache, CACHE_TTL_SECONDS
except ImportError:
...
def _make_response(data: Any = None, error: str = "", status: int = 200) -> dict:
    ...
    resp = {"ok": status < 400, "timestamp": datetime.now().isoformat()}
    if error:
        resp["error"] = error
    if data is not None:
        resp["data"] = data
...
try:
    from .gateway_telemetry import setup_telemetry, get_telemetry, telemetry_middleware
...
try:
    from .gateway_events import EventBus, get_event_bus
...
try:
    from core.autotune import AutoTuneEngine, TuneProfile, ContextCategory
...
try:
    from core.parallel_racer import ParallelRacer, RacingTier
...
class AntigravityGateway(
    ...
    def __init__(self, config: GatewayConfig):
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=_pool_workers, thread_name_prefix="gw"
        )
        self._ready = False
    ...
    async def middleware_auth(self, request: web.Request, handler) -> web.Response:
        ...
        public_paths = {"/v1/health", "/v1/ready", "/v1/"}
        if request.path in public_paths or not self.config.require_auth:
            return await handler(request)
    ...
    async def middleware_rate_limit(self, request: web.Request, handler) -> web.Response:
        ...
        if request.path in {"/v1/health", "/v1/ready", "/v1/metrics", "/v1/mem0/stats"}:
            return await handler(request)
        ...
        if self._sliding_rate_limiter is not None:
            result = self._sliding_rate_limiter.check(client_ip)
    ...
    async def handle_session_report(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return web.json_response({"stored": result, "project": project})
        except Exception as exc:
    ...
    def build_app(self) -> web.Application:
        ...
        app = web.Application(
            ...
            client_max_size=1024 * 1024,  # 1MB max body
        )
        ...
        app.router.add_get(
            ...
            lambda r: web.json_response(
                {
                    "message": "MCP endpoint active. Use POST for JSON-RPC or /v1/mcp/sse for SSE transport."
                },
                status=200,
            ),
        )
        app.router.add_post("/v1/mcp", self.handle_mcp)  # Direct JSON-RPC
        # MCP SSE removido — StreamResponse bloquea middleware y genera CLOSE_WAIT
        # Los IDEs usan JSON-RPC directo (POST /v1/mcp) o stdio transport
        app.router.add_post("/v1/mcp/message", self.handle_mcp_message)  # Message POST
        ...
        app.router.add_get(
            "/v1/context-engine/current", self.handle_context_engine_current
        )
        app.router.add_post(
            "/v1/context-engine/switch", self.handle_context_engine_switch
        )
        app.router.add_post(
            "/v1/context-engine/preview", self.handle_context_engine_preview
        )
        app.router.add_post(
            "/v1/context-engine/stats", self.handle_context_engine_stats
        )
...
async def route_excel_close(request: web.Request) -> web.Response:  # type: ignore[no-untyped-def]
    ...
    return web.json_response(
        _excel().handle_excel_close({"session_id": sid, "save": save_q})
    )
...
def build_app_for_tests(
    ...
    async def auth_mw(
        ...
        public_paths = {"/v1/health", "/v1/ready"}
...
def main() -> None:
    ...
    if config.host == "0.0.0.0" and not config.require_auth:
        log.error(
