...
class EventBus:
    ...
    async def _notify_listeners(self, channel: str, data: dict[str, Any]) -> int:
        """Notifica a los listeners de codigo cuyo patron coincide con el canal.
        ...
        Returns:
            Cantidad de listeners notificados exitosamente.
        """
        notified = 0
    ...
    async def _emit_to_websocket(self, channel: str, data: dict[str, Any], source: str) -> int:
        """Emite el evento al transporte WebSocket si corresponde.

        No reenvia eventos cuyo origen ya es 'websocket' para evitar loops.
        ...
        Returns:
            Cantidad de clientes WebSocket notificados.
        """
        if source == "websocket":
            return 0
        try:
            ...
            return await ws_manager.broadcast(channel, data)
        except ImportError:
            return 0
        except Exception as exc:
    ...
    async def _emit_to_sse(self, channel: str, data: dict[str, Any], source: str) -> int:
        """Emite el evento al transporte SSE si corresponde.

        No reenvia eventos cuyo origen ya es 'sse' para evitar loops.
        ...
        Returns:
            Cantidad de clientes SSE notificados (0 o 1).
        """
        if source == "sse":
            return 0
        try:
    ...
    def _record_event(
        self, channel: str, data: dict[str, Any], timestamp: str, source: str, notified: int
    ) -> None:
        """Registra el evento en el historial limitado del bus.

        Args:
            channel: Canal del evento.
            data: Datos del evento.
            timestamp: Marca temporal ISO del evento.
            source: Origen del evento.
            notified: Cantidad total de receptores notificados.
        """
        record = EventRecord(
            channel=channel,
            data=data,
    ...
    async def emit(
        ...
        notified = await self._notify_listeners(channel, data)
        notified += await self._emit_to_websocket(channel, data, source)
        notified += await self._emit_to_sse(channel, data, source)

        self._record_event(channel, data, timestamp, source, notified)
