...
class _AgentsMixin:
    ...
    async def handle_reload_agents(self, request: web.Request) -> web.Response:
        ...
        log.info(
            "Agents cache reload: %d agentes totales, %d ejecutables", len(agents), len(executable)
        )
    ...
    async def handle_team_message(self, request: web.Request) -> web.Response:
        ...
        if not _validate_name(team_id):
            return web.json_response(
                _make_response(error="ID de equipo invalido", status=400),
                status=400,
            )
