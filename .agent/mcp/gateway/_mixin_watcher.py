...
def _format_prometheus() -> str:
    ...
    for importance, count in _METRICS_BY_IMPORTANCE.items():
        lines.append(f'watcher_events_by_importance_total{{importance="{importance}"}} {count}')
    return "\n".join(lines) + "\n"
...
# Recent keys para dedup: timestamp por (watch_id, label, line_number).
_dedup_cache: dict[tuple[str, str, int], float] = {}
...
def _gc_dedup_cache(now: float) -> None:
    """Limpia entradas fuera de la ventana de dedup cada ~5s."""
    global _dedup_cache, _dedup_cache_mtime
    if now - _dedup_cache_mtime > 5.0:
        cutoff = now - _DEDUP_WINDOW_SEC
        _dedup_cache = {
            key: timestamp for key, timestamp in _dedup_cache.items() if timestamp >= cutoff
        }
...
def _should_drop_duplicate(event: dict[str, Any]) -> bool:
    ...
    if previous is not None and now - previous < _DEDUP_WINDOW_SEC:
        return True
...
class _WatcherMixin:
    ...
    async def handle_watcher_events(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return web.json_response(
                err("watcher.json_invalid", f"json invalido: {exc}", source="watcher"),
                status=400,
            )
        if not isinstance(raw, dict):
            return web.json_response(
                err("watcher.body_invalid", "body debe ser objeto", source="watcher"),
                status=400,
            )
        ...
        if not event["watch_id"]:
            return web.json_response(
                err("watcher.param_missing", "watch_id es requerido", source="watcher"),
                status=400,
            )
        ...
        if _should_drop_duplicate(event):
            ...
            return web.json_response(
                ok({"dropped": "duplicate", "event": event}, source="watcher"),
                status=202,
            )
        if not _rate_limit_ok(event["watch_id"], event["importance"]):
            _METRICS["events_dropped_rate_limit_total"] += 1
            return web.json_response(
                ok({"dropped": "rate_limit", "event": event}, source="watcher"),
                status=429,
            )
        ...
        notified = 0
        try:
            notified += await bus.emit(f"watcher.{importance}", event, source="watcher")
            notified += await bus.emit("watcher.match", event, source="watcher")
        except Exception as exc:  # noqa: BLE001
            log.warning("EventBus emit watcher fallo: %s", exc)
            return web.json_response(
                err("watcher.bus_error", f"bus emit: {exc}", source="watcher"),
                status=500,
            )
        ...
        return web.json_response(
            ok({"notified": notified, "event": event}, source="watcher"), status=200
        )

    async def handle_watcher_events_recent(self, request: web.Request) -> web.Response:
        ...
        channel = (
            f"watcher.{importance}" if importance in {"low", "high", "critical"} else "watcher.*"
        )
        ...
        try:
            ...
            return web.json_response(
                err("watcher.history_error", f"history: {exc}", source="watcher"),
                status=500,
            )

        return web.json_response(ok({"events": history, "count": len(history)}, source="watcher"))

    def _resolve_sse_cors_origin(self, origin: str) -> str | None:
        """Resuelve el valor de Access-Control-Allow-Origin para el SSE stream.

        Lee la lista de origenes permitidos desde la config (poblada por la env
        var ANTIGRAVITY_CORS_ORIGINS). Si la lista incluye el wildcard "*",
        devuelve "*" (con warning una sola vez). Si no, refleja el Origin del
        request cuando esta permitido; como fallback usa el primer origen de la
        lista para no romper clientes locales del ecosistema.

        Args:
            origin: Header Origin del request entrante (puede ser "").

        Returns:
            El valor a usar en el header, o None si no hay origenes configurados.
        """
        # Importar desde el modulo liviano security_utils (solo os/pathlib/urllib),
        # NO desde _gateway_main: ese arrastra la cadena pesada
        # (_gateway_main -> dpapi -> cryptography -> _cffi_backend), lo que hacia
        # CRASHEAR el handler SSE en entornos sin cryptography/cffi (ej. el job de
        # CI "watcher + context engine" que solo instala aiohttp). El crash se veia
        # como ServerDisconnectedError en los tests de stream. gateway_main importa
        # la misma funcion desde aca.
        from ..security_utils import is_origin_allowed

        config = getattr(self, "config", None)
        allowed = list(getattr(config, "cors_origins", []) or [])
        if not allowed:
            allowed = ["*"]

        if "*" in allowed:
            if not getattr(self, "_cors_wildcard_warned", False):
                log.warning(
                    "CORS del watcher SSE configurado con wildcard '*'. "
                    "Setea ANTIGRAVITY_CORS_ORIGINS con una lista explicita "
                    "de origenes para entornos no locales."
                )
                self._cors_wildcard_warned = True
            return "*"

        if origin and is_origin_allowed(origin, allowed):
            return origin
        # Fallback seguro: primer origen permitido de la lista (no wildcard).
        return allowed[0]
    ...
    async def handle_watcher_stream(self, request: web.Request) -> web.StreamResponse:
        ...
        # CORS del SSE: en vez de wildcard fijo, resolver el header desde la
        # config (alimentada por ANTIGRAVITY_CORS_ORIGINS). Mantiene a los
        # clientes locales (Nexus, browser dev) reflejando su Origin permitido,
        # y solo cae a "*" si la env var lo habilita explicitamente.
        sse_headers = {
        ...
        allow_origin = self._resolve_sse_cors_origin(request.headers.get("Origin", ""))
        if allow_origin:
            sse_headers["Access-Control-Allow-Origin"] = allow_origin
            if allow_origin != "*":
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await response.write(f"event: watcher\ndata: {payload}\n\n".encode())
                except TimeoutError:
    ...
    async def handle_watcher_metrics(self, request: web.Request) -> web.Response:
        ...
        if "application/json" in accept:
            return web.json_response(ok(_metrics_snapshot(), source="watcher"))
    ...
    async def handle_watcher_events_history(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(ok({"events": events, "count": len(events)}, source="watcher"))
    ...
    async def handle_watcher_spawn(self, request: web.Request) -> web.Response:
        ...
        if not cmd:
            return web.json_response(
                err("watcher.param_missing", "cmd es requerido", source="watcher"),
                status=400,
            )
        ...
        try:
            watch_id = watcher.spawn(
                ...
                env={str(k): str(v) for k, v in (body.get("env") or {}).items()} or None,
                label=body.get("label") or None,
            )
        except Exception as exc:  # noqa: BLE001
            return web.json_response(
                err("watcher.spawn_failed", str(exc), source="watcher"),
                status=400,
            )
        return web.json_response(ok({"watch_id": watch_id}, source="watcher"))
    ...
    async def handle_watcher_kill_http(self, request: web.Request) -> web.Response:
        ...
        if not watch_id:
            return web.json_response(
                err("watcher.param_missing", "watch_id requerido", source="watcher"),
                status=400,
            )
        watcher = _get_watcher()
        try:
            killed = watcher.kill(watch_id)
        except Exception as exc:  # noqa: BLE001
            return web.json_response(
                err("watcher.kill_failed", str(exc), source="watcher"),
                status=404,
            )
        return web.json_response(ok({"killed": killed, "watch_id": watch_id}, source="watcher"))
    ...
    async def handle_watcher_list_http(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(ok({"watches": watches}, source="watcher"))
    ...
    async def handle_watcher_tail_http(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return web.json_response(
                err("watcher.tail_failed", str(exc), source="watcher"),
                status=404,
            )
        return web.json_response(ok(data, source="watcher"))
