...
def _build_agent_tools(all_agents: list[dict], agents_limit: int, search: str) -> list[dict]:
    """Construye la lista de agentes en formato tool LLM.

    Args:
        all_agents: Agentes disponibles del ecosistema.
        agents_limit: Cantidad máxima de agentes a incluir.
        search: Término de búsqueda en minúsculas (vacío = sin filtro).

    Returns:
        Lista de diccionarios con la representación tool de cada agente.
    """
...
def _build_skill_tools(all_skills: list[dict], skills_limit: int, search: str) -> list[dict]:
    """Construye la lista de skills en formato tool LLM.

    Args:
        all_skills: Skills disponibles del ecosistema.
        skills_limit: Cantidad máxima de skills a incluir.
        search: Término de búsqueda en minúsculas (vacío = sin filtro).

    Returns:
        Lista de diccionarios con la representación tool de cada skill.
    """
...
class _SkillsMixin:
    ...
    async def handle_execute_skill(self, request: web.Request) -> web.Response:
        ...
        if executor:
            return await self._execute_skill_via_agent(
                executor=executor,
        ...
        return await self._execute_skill_via_script(
            skill_dir=skill_dir,
            skill_name=skill_name,
            skill_source=skill_source,
    ...
    async def _execute_skill_via_agent(
        self,
        *,
        executor: Any,
        skill_name: str,
        ...
        """Ejecuta un skill delegando en el mejor agente disponible.

        Args:
            executor: Executor de agentes ya resuelto.
            skill_name: Nombre del skill a ejecutar.
            skill_source: Origen del skill ("main" o "custom").
            task: Tarea formateada con el contexto del skill.
            timeout: Timeout en segundos para la ejecución del agente.
            llm_config: Configuración opcional de LLM a pasar al agente.
            persona: Persona opcional a usar en la ejecución.

        Returns:
            Respuesta HTTP con el resultado del agente o el error correspondiente.
        """
    ...
    async def _execute_skill_via_script(
        ...
        *,
        ...
        Args:
            ...
            skill_name: Nombre del skill a ejecutar.
            skill_source: Origen del skill ("main" o "custom").
            user_input: Input del usuario que se pasa como argumento al script.
            timeout: Timeout en segundos para la ejecución del subproceso.

        Returns:
            Respuesta HTTP con la salida del script o el error correspondiente.
        """
        from .._gateway_main import _make_response, _sanitize_error
    ...
    async def _load_manifest_skills(self, filter_type: str) -> list[dict]:
        """Carga la lista de skills para el manifest de herramientas (con cache).

        Args:
            filter_type: Filtro solicitado ("all", "skills" o "agents").

        Returns:
            Lista de skills si el filtro los incluye, lista vacía en caso contrario.
        """
        if filter_type not in ("all", "skills"):
            return []
    ...
    async def _load_manifest_agents(self, filter_type: str) -> list[dict]:
        """Carga la lista de agentes para el manifest de herramientas.

        Args:
            filter_type: Filtro solicitado ("all", "skills" o "agents").

        Returns:
            Lista de agentes si el filtro los incluye, lista vacía en caso contrario.
        """
        if filter_type not in ("all", "agents"):
            return []
        executor = self.executor or await self.get_executor()
        if executor:
            return executor.get_available_agents()
        return self._list_agents_from_filesystem()
    ...
    async def handle_tools_manifest(self, request: web.Request) -> web.Response:
        ...
        all_skills = await self._load_manifest_skills(filter_type)
        all_agents = await self._load_manifest_agents(filter_type)
        ...
        agent_tools = _build_agent_tools(all_agents, agents_limit, search)
        skill_tools = _build_skill_tools(all_skills, skills_limit, search)
