...
class WatcherEventStore:
    ...
    def __init__(self, db_path: Path | None = None, ttl_days: int | None = None) -> None:
        ...
        if ttl_days is None:
            try:
                ttl_days = int(os.environ.get("ANTIGRAVITY_WATCHER_HISTORY_TTL_DAYS", "30") or 30)
            except ValueError:
                ttl_days = 30
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            with self._conn() as conn:
                cur = conn.execute("DELETE FROM watcher_events WHERE received_at < ?", (cutoff,))
    ...
    def stats(self) -> dict[str, Any]:
        ...
        try:
            with self._conn() as conn:
                total = conn.execute("SELECT COUNT(*) AS n FROM watcher_events").fetchone()["n"]
                by_importance = {
                    r["importance"]: r["n"]
                    for r in conn.execute(
                        "SELECT importance, COUNT(*) AS n FROM watcher_events GROUP BY importance"
                    )
                }
                oldest = conn.execute(
