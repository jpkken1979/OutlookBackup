...
# Forzar carga offline del modelo HuggingFace — evita requests HTTP a HF Hub en cada inicio.
# El modelo multi-qa-MiniLM-L6-cos-v1 ya esta en cache local; sin esto tarda ~12s vs 0.3s.
...
def _get_validated_ollama_base() -> str:
    ...
    # SECURITY: Validate hostname against SSRF (private IPs, localhost)
    from .security_utils import validate_url_for_ssrf
    is_valid, reason = validate_url_for_ssrf(url)
    if not is_valid:
        raise ValueError(f"OLLAMA_BASE_URL SSRF bloqueada: {reason}")
    return url
...
def _ensure_initialized() -> bool:
    ...
    # Retry loading SentenceTransformer — Windows can fail with [Errno 22]
    # when antivirus/indexer holds a lock on the HuggingFace cache files.
    import time
    ...
    for attempt in range(3):
        try:
            ...
            logger.info(
                "SentenceTransformer cargado: %s (intento %d)", _HF_EMBED_MODEL, attempt + 1
            )
            _embed_loaded = True
            break
        except Exception as e:
            logger.warning("SentenceTransformer intento %d fallo: %s", attempt + 1, e)
            if attempt < 2:
                time.sleep(2)

    if not _embed_loaded:
        _INIT_ERROR = "sentence-transformers no disponible tras 3 intentos"
        logger.error(_INIT_ERROR)
        _collection = None
        _chroma_client = None
        return False
...
def handle_memory_store(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if _mem0_mode and _mem0_instance is not None and not skip_enhanced:
        try:
            ...
            # Loguear explícitamente la caída a modo base para que sea visible en logs
            logger.warning("mem0 store fallo -- cayendo a chromadb-direct (motivo: %s)", e)
    ...
    try:
        ...
        # Store additional metadata keys if they are scalar
        for k, v in metadata.items():
            if k not in chroma_meta and isinstance(v, (str, int, float, bool)):
                chroma_meta[k] = v
        ...
        logger.error("memory_store error: %s", e)
        return {"error": str(e)}
...
def handle_memory_stats(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.debug("User filter failed in stats, falling back to total")
            user_count = total  # If filter fails, use total
...
def main() -> None:
    ...
    # Eagerly initialize to surface errors early
    if _ensure_initialized():
        if _mem0_mode:
            logger.info(
                "Modo MEJORADO activo: mem0 + Ollama (%s) + ChromaDB + sentence-transformers",
                _OLLAMA_MODEL,
            )
        else:
            logger.info("Modo BASE activo: ChromaDB + sentence-transformers (sin LLM)")
            logger.info(
                "  -> Para activar extraccion de hechos: instalar Ollama y ejecutar 'ollama serve'"
            )
    else:
        logger.warning("Backend no disponible: %s", _INIT_ERROR)
