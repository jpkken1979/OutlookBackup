...
def _truncate_preview(text: str, limit: int) -> str:
    """Trunca un texto a un limite agregando puntos suspensivos si excede.

    Args:
        text: Texto a truncar.
        limit: Cantidad maxima de caracteres antes de truncar.

    Returns:
        Texto truncado con sufijo '...' si supero el limite.
    """
    return text[:limit] + ("..." if len(text) > limit else "")


def _extract_model_parameters(model_info: dict) -> dict:
    """Extrae los parametros clave del bloque model_info de Ollama.

    Args:
        model_info: Diccionario model_info devuelto por /api/show.

    Returns:
        Diccionario con los parametros clave presentes (puede estar vacio).
    """
...
def _build_model_info_result(model: str, resp: dict) -> dict:
    """Arma el diccionario de informacion detallada de un modelo.

    Args:
        model: Nombre del modelo consultado.
        resp: Respuesta cruda de /api/show de Ollama.

    Returns:
        Diccionario con detalles, previews y parametros del modelo.
    """
    ...
    if template:
        result["template_preview"] = _truncate_preview(template, 300)

    system = resp.get("system", "")
    if system:
        result["system_prompt"] = _truncate_preview(system, 300)

    license_text = resp.get("license", "")
    if license_text:
        result["license_preview"] = _truncate_preview(license_text, 200)

    key_params = _extract_model_parameters(resp.get("model_info", {}))
    if key_params:
...
def tool_ollama_model_info(model: str) -> str:
    ...
    result = _build_model_info_result(model, resp)
    return json.dumps(result, indent=2, ensure_ascii=False)
...
class _MissingParam(Exception):  # noqa: N818
    """Señala que faltan parámetros requeridos para una tool.

    Se usa internamente por los handlers de tools para abortar con un mensaje
    de error específico sin tener que construir la respuesta JSON-RPC ellos
    mismos; :func:`_dispatch_tool_call` la captura y la traduce a un error.

    Args:
        message: Mensaje de error a mostrar al cliente.
    """


def _run_ollama_status(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_status`` (sin parámetros)."""
    return tool_ollama_status()


def _run_ollama_list_models(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_list_models`` (sin parámetros)."""
    return tool_ollama_list_models()


def _run_ollama_pull_model(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_pull_model`` validando el parámetro ``model``."""
    model = args.get("model", "")
    if not model:
        raise _MissingParam("Parámetro 'model' requerido")
    return tool_ollama_pull_model(model)


def _run_ollama_generate(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_generate`` validando ``model`` y ``prompt``."""
    model = args.get("model", "")
    prompt = args.get("prompt", "")
    if not model or not prompt:
        raise _MissingParam("Parámetros 'model' y 'prompt' requeridos")
    return tool_ollama_generate(
        model=model,
        prompt=prompt,
...
def _run_ollama_chat(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_chat`` validando ``model`` y ``messages``."""
    model = args.get("model", "")
    messages = args.get("messages", [])
    if not model or not messages:
        raise _MissingParam("Parámetros 'model' y 'messages' requeridos")
    return tool_ollama_chat(
        model=model,
        messages=messages,
        temperature=float(args.get("temperature", 0.7)),
        max_tokens=int(args.get("max_tokens", 2048)),
...
def _run_ollama_embeddings(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_embeddings`` validando el parámetro ``text``."""
    text = args.get("text", "")
    if not text:
        raise _MissingParam("Parámetro 'text' requerido")
    return tool_ollama_embeddings(
        text=text,
        model=args.get("model", "nomic-embed-text"),
    )


def _run_ollama_model_info(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_model_info`` validando el parámetro ``model``."""
    model = args.get("model", "")
    if not model:
        raise _MissingParam("Parámetro 'model' requerido")
    return tool_ollama_model_info(model)


def _run_ollama_benchmark(args: dict[str, Any]) -> str:
    """Ejecuta ``ollama_benchmark`` validando el parámetro ``model``."""
    model = args.get("model", "")
    if not model:
        raise _MissingParam("Parámetro 'model' requerido")
    return tool_ollama_benchmark(model)


# Mapa de tools soportadas: nombre -> handler que valida args y devuelve texto.
_TOOL_HANDLERS: dict[str, Callable[[dict[str, Any]], str]] = {
    "ollama_status": _run_ollama_status,
    "ollama_list_models": _run_ollama_list_models,
    "ollama_pull_model": _run_ollama_pull_model,
    "ollama_generate": _run_ollama_generate,
    "ollama_chat": _run_ollama_chat,
    "ollama_embeddings": _run_ollama_embeddings,
    "ollama_model_info": _run_ollama_model_info,
    "ollama_benchmark": _run_ollama_benchmark,
}


def _dispatch_tool_call(request_id: Any, name: str, args: dict[str, Any]) -> dict[str, Any]:
    """Ejecuta la tool Ollama solicitada por ``tools/call``.

    Resuelve la tool por nombre vía :data:`_TOOL_HANDLERS`, delega la validación
    de parámetros al handler correspondiente y envuelve el resultado o el error
    en una respuesta JSON-RPC.

    Args:
        request_id: Id del request JSON-RPC a reflejar en la respuesta.
        name: Nombre de la tool solicitada.
    ...
    try:
        ...
        return _error_result(request_id, str(e))
    except Exception as e:
...
def handle_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if method == "tools/call":
        ...
        return _dispatch_tool_call(request_id, name, args)
