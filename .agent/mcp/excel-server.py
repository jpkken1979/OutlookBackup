...
def handle_excel_update_dashboard(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return (
        _ensure_engine()
        .update_dashboard(
            sid, dashboard_sheet=dashboard_sheet, data_sheets=data_sheets, refresh_all=refresh_all
        )
        .to_dict()
    )
...
def _mcp_tools_payload() -> list[dict[str, Any]]:
    """Construye la lista de tools en formato MCP a partir de METHODS.

    Returns:
        Lista de dicts con name, description e inputSchema por tool.
    """
    tools: list[dict[str, Any]] = []
    for name, handler in METHODS.items():
        doc = (handler.__doc__ or name).strip().splitlines()[0]
        tools.append(
            {
                "name": name,
                "description": doc,
...
def handle_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    Soporta el protocolo MCP (initialize, tools/list, tools/call) y, por
    compatibilidad con el gateway, el dispatch directo legacy (excel_*).
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "antigravity-excel", "version": "1.0.0"},
            },
        }

    if method == "tools/list":
        return {"jsonrpc": "2.0", "id": rid, "result": {"tools": _mcp_tools_payload()}}

    if method == "tools/call":
        tool_name = params.get("name", "")
        tool_handler = METHODS.get(tool_name)
        if tool_handler is None:
            return {
                "jsonrpc": "2.0",
                "id": rid,
                "error": {"code": -32601, "message": f"tool desconocida: {tool_name}"},
            }
        try:
            result = tool_handler(params.get("arguments", {}) or {})
        except Exception as exc:  # noqa: BLE001
            logger.exception("tool %s lanzó excepción", tool_name)
