...
## PASO 1.5 — Verificar divergencia vs origin y commits locales sin push
...
- Si la rama está **atrás de origin**: `git pull` antes de continuar para traer los cambios remotos.
- Si está **adelantada y atrás** (divergente): informar al usuario antes de continuar.
- Si está **solo adelantada** (existen commits locales sin push, listados por `git cherry`): se pushearán automáticamente al final del flujo.
...
### Si cambiaron configs MCP o el injector (`.mcp.json`, `.cursor/`, `.windsurf/`, `.vscode/`, `.zed/`, `.agent/scripts/mcp_injector.py`, `.agent/scripts/regen_mcp_configs.py`):
```bash
python -m pytest tests/scripts/test_mcp_injector_portability.py -q 2>&1 | tail -20
```
Detecta drift de las 6 configs versionadas: conteo de servers incorrecto (22, Zed 20)
o paths de usuario hardcodeados (`C:\Users\<nombre>` en vez de `${USERPROFILE}`/`${ANTIGRAVITY_ROOT}`).
Si falla, regenerá con `make mcp` o derivá del `.mcp.json` canónico. Ver `docs/mcp-config-multiplataforma.md`.
...
## PASO 4.3 — Refrescar conteos de CLAUDE.md (idempotente)

Antes de que el crawler ingeste docs, asegurar que los datos cuantitativos de
`CLAUDE.md` (versiones canonicas y conteos de inventario) reflejen el estado real
del filesystem:

```bash
python .agent/scripts/refresh_claude_md.py --apply
```

Comportamiento esperado:
- **Idempotente**: si los conteos no cambiaron, el script no toca el archivo
  (devuelve `Sin cambios — CLAUDE.md ya esta sincronizado.`).
- Si CLAUDE.md cambia, queda incluido en el commit del PASO 5 (no requiere commit aparte).
- Solo refresca bloques marcados con `<!-- AUTO:versions -->` y `<!-- AUTO:counts -->`.

**No bloquea el flujo** si el script falla — reportar el warning y seguir.
...
Ver `docs/rules-reference/repo-crawler.md` para política completa de tags y paths.
